...
static void skel_hash_init(afl_state_t *afl, u32 cap) {
  if (!cap) { cap = 4096; }
  // force power-of-two
  u32 p = 1;
  while (p < cap) { p <<= 1; }
  afl->skel_hash_cap = p;
  afl->skel_hash_count = 0;
  afl->skel_hash_table = ck_alloc(sizeof(u64) * afl->skel_hash_cap);
  memset(afl->skel_hash_table, 0, sizeof(u64) * afl->skel_hash_cap);
}

static u64 saved_non_instantiable_graphs = 0;

void save_non_instantiable_skeleton_graph(afl_state_t *afl,
                                             const struct SkeletonGraph *graph,
                                             u8 exit_code,
                                             u32 graph_id) {

  if (unlikely(!afl || !graph)) { return; }

  char *dir = alloc_printf("%s/non_instantiable", afl->out_dir);
  ...
  u32 src_id = afl->queue_cur ? afl->queue_cur->id : 0;
  u8  fn[PATH_MAX];
  snprintf((char *)fn, PATH_MAX,
           "%s/non_instantiable/id:%06llu,src:%06u,gid:%06u,exit:%02u.json",
           afl->out_dir, saved_non_instantiable_graphs, src_id, graph_id,
           exit_code);
...
static void skel_hash_rehash(afl_state_t *afl, u32 new_cap) {
  u64 *old_table = afl->skel_hash_table;
  u32 old_cap = afl->skel_hash_cap;

  skel_hash_init(afl, new_cap);
  if (!old_table) { return; }

  for (u32 i = 0; i < old_cap; i++) {
    u64 key = old_table[i];
    if (!key) { continue; }
    u32 mask = afl->skel_hash_cap - 1;
    u32 idx = (u32)key & mask;
    while (afl->skel_hash_table[idx]) {
      idx = (idx + 1) & mask;
    }
    afl->skel_hash_table[idx] = key;
    afl->skel_hash_count++;
  }
...
bool skeleton_graph_seen_or_add(afl_state_t *afl, const struct SkeletonGraph *graph) {
  if (!afl || !graph) { return false; }
  ...
  if (!afl->skel_hash_table || afl->skel_hash_cap == 0) {
    skel_hash_init(afl, 4096);
  }

  if (afl->skel_hash_count * 100 >= afl->skel_hash_cap * 70) {
    skel_hash_rehash(afl, afl->skel_hash_cap << 1);
  }

  u32 mask = afl->skel_hash_cap - 1;
  u32 idx = (u32)key & mask;
  while (1) {
    u64 slot = afl->skel_hash_table[idx];
    if (!slot) {
      afl->skel_hash_table[idx] = key;
      afl->skel_hash_count++;
      return false;
...
u8 calibrate_case(afl_state_t *afl, struct queue_entry *q, u8 *use_mem,
  ...
  */
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (WIFEXITED(afl->fsrv.child_status)) {
      int exit_code = WEXITSTATUS(afl->fsrv.child_status);
      if (exit_code == EXIT_EVENT_MISMATCH ||
        ...
        if (q->graph_data) {
          ...
          reduce_queue_perf_score_hard(q);
        }
        fault = FSRV_RUN_NOINST;
        goto abort_calibration;
      }
    ...
    for (o = m; o < n; o++) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        if (likely(next_min_accept < max_start_id ||
          ...
          afl->queued_imported += save_if_interesting(afl, mem, new_len, fault, 0, MAP_SIZE);
          show_stats(afl);
          afl->syncing_party = 0;
          munmap(mem, st.st_size);
...
size_t write_to_temp_json(afl_state_t *afl, struct SkeletonGraphData *sgi){

  char *filename = alloc_printf("%s/queue/id:%06u,src:%06u.json", afl->out_dir, sgi->id, afl->queue_cur->id);
...
static void race_set_init(afl_state_t *afl) {

  if (afl->race_set_size) { return; }
  afl->race_set_size = 1024;
  afl->race_set_count = 0;
  afl->race_set_hashes = ck_alloc(afl->race_set_size * sizeof(u64));
  memset(afl->race_set_hashes, 0, afl->race_set_size * sizeof(u64));

}

static void race_set_grow(afl_state_t *afl) {

  u32 old_size = afl->race_set_size;
  u64 *old = afl->race_set_hashes;

  afl->race_set_size = old_size ? old_size << 1 : 1024;
  afl->race_set_hashes = ck_alloc(afl->race_set_size * sizeof(u64));
  memset(afl->race_set_hashes, 0, afl->race_set_size * sizeof(u64));
  afl->race_set_count = 0;
  ...
  for (u32 i = 0; i < old_size; ++i) {
    ...
    if (!old[i]) { continue; }
    u64 key = old[i];
    u32 pos = (u32)(key & (afl->race_set_size - 1));
    while (afl->race_set_hashes[pos]) {
      pos = (pos + 1) & (afl->race_set_size - 1);
    }
    afl->race_set_hashes[pos] = key;
    ++afl->race_set_count;
...
static bool race_set_add(afl_state_t *afl, u64 hash) {

  if (!afl->race_set_size) { race_set_init(afl); }

  if (afl->race_set_count * 4 >= afl->race_set_size * 3) {
    race_set_grow(afl);
  }
  ...
  while (afl->race_set_hashes[pos]) {
    ...
    if (afl->race_set_hashes[pos] == key) { return false; }
    pos = (pos + 1) & (afl->race_set_size - 1);
...
void save_race_if_interesting(afl_state_t *afl, const struct SkeletonGraphData *sgi) {

  if (!afl->check_data_race || !sgi || !sgi->is_racy) { return; }
  ...
  if (!race_set_add(afl, hash)) { return; }

  if (afl->saved_races >= KEEP_UNIQUE_RACE) { return; }

  u8 fn[PATH_MAX];
  snprintf(fn, PATH_MAX, "%s/races/id:%06llu,src:%06u", afl->out_dir,
           afl->saved_races, sgi->id);
  ...
  ++afl->saved_races;
  afl->last_race_time = get_cur_time();
  afl->last_race_execs = afl->fsrv.total_execs;
...
u8 skeleton_graph_fuzz_stuff(afl_state_t *afl, struct SkeletonGraphData *sgi){
  ...
  if (unlikely(write_to_testcase(afl, &exec_mem, exec_len, 1) == 0)) {
  ...
  if (fault != FSRV_RUN_OK) {
    ...
    save_if_interesting_skeleton(afl, sgi->skeleton_graph, fault);
    return fault;
  }
  if (WIFEXITED(afl->fsrv.child_status)) {
    sgi->already_simulated = 1;
    exit_code = WEXITSTATUS(afl->fsrv.child_status);
    ...
    if (exit_code == EXIT_EVENT_MISMATCH || exit_code == EXIT_RF_TYPE_MISMATCH || exit_code == EXIT_NOT_INSTANTIABLE) {
      // save_non_instantiable_skeleton_graph(afl, sgi->skeleton_graph,(u8)exit_code, sgi->id);
      // Not required as we will not save this anyways. - Hardik
      // sgi->non_instantiable = 1;
      // ACTF("Flagged entry %u due to simulator exit %d", afl->queue_cur->id,exit_code);
  ...
  if(afl->enable_feedback && afl->queue_cur && afl->queue_cur->graph_data){
    if(!next_events_from_shm){
    ...
    if(!next_events_from_shm){
      ...
      // ACTF("[SHM NEXT EVENTS FEEDBACK] updating the simulator feedback cache for queue_cur, graph_data id: %d", afl->queue_cur->graph_data->id);
...
u8 __attribute__((hot)) common_fuzz_stuff(afl_state_t *afl, u8 *out_buf,
  ...
  afl->queued_discovered += save_if_interesting(afl, out_buf, len, fault, 0, MAP_SIZE);
  // ACTF("* queue_discovered = %u *", afl->queued_discovered);
