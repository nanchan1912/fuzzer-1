...
- `skeleton_graph_mutator.cpp` - skeleton mutation logic and static program abstraction parsing/ownership (`cfg_new`, `eg_file`)
- `skeleton_potential.cpp` - Mode 1 potential calculation and incremental update logic only
- `skeleton_potential_2.cpp` - Mode 2 potential calculation and incremental update logic only
- `potential_nn.cpp` - nearest-neighbor index for Mode 1 potential scoring
- `potential_nn_2.cpp` - nearest-neighbor index for Mode 2 potential scoring

## Cohesion Note

Potential modules now use shared static abstraction definitions from `include/static_program_abstraction.hpp`.
