    ...
    if (*lptr == ':') {
      ...
      *lptr = 0;
      ...
      if (starts_with("saved_races", keystring)) {
        ...
        afl->saved_races = strtoull(lptr, &nptr, 10);

      }

      if (starts_with("last_race", keystring)) {

        afl->last_race_time = strtoull(lptr, &nptr, 10) * 1000;
...
void write_stats_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  if (afl->check_data_race) {
    ...
    fprintf(f,
            ...
            afl->saved_races, afl->last_race_time / 1000,
            afl->fsrv.total_execs - afl->last_race_execs);
...
void maybe_update_plot_file(afl_state_t *afl, u32 t_bytes, double bitmap_cvg,
  ...
  if((afl->plot_prev_mo_cov == afl->mo_coverage) &&
     (afl->plot_prev_rf_cov == afl->rf_coverage)) {
  ...
  afl->plot_prev_md = afl->max_depth;
  afl->plot_prev_ed = afl->fsrv.total_execs;
  afl->plot_prev_mo_cov = afl->mo_coverage;
  afl->plot_prev_rf_cov = afl->rf_coverage;
  ...
  fprintf(afl->fsrv.plot_file,
          ...
          (u32)afl->san_binary_length, afl->mo_coverage, afl->rf_coverage);                    /* ignore errors */
...
#define SP20 SP10 SP10
  ...
  char *crash_color = afl->total_crashes   ? cLRD
                      : afl->saved_crashes ? cYEL
                                           : cRST;
  char *race_color = afl->total_races   ? cLRD
                     : afl->saved_races ? cYEL
                                        : cRST;
  ...
  SAYF(bV bSTOP " last saved hang : " cRST "%-33s " bSTG bV bSTOP
      ...
      if (afl->check_data_race) {
        ...
        sprintf(tmp, "%s%s", u_stringify_int(IB(0), afl->saved_races),
          (afl->saved_races >= KEEP_UNIQUE_RACE) ? "+" : "");

        u_stringify_time_diff(time_tmp, cur_ms, afl->last_race_time);
  ...
  SAYF(SET_G1 "\n" bSTG bLB bH            cCYA bSTOP " mo coverage:" cPIN
       ...
       u_stringify_int(IB(0), afl->mo_coverage),
       u_stringify_int(IB(1), afl->rf_coverage),
       afl->current_phase == MO_FOOTPRINT_DRIVEN_PHASE
           ? "MO_FOOTPRINT_DRIVEN_PHASE"
           : (afl->current_phase == POTENTIAL_DRIVEN_PHASE ? "POTENTIAL_DRIVEN_PHASE" : "UNKNOWN"));
