...
set<EventID> find_consistent_writes(const SkeletonGraph& graph,
    ...
    while (!to_explore.empty()) {
        ...
        // Condition 1: Found a WRITE or RMW on the same location - this is a aware_of point
        // RMW events are both reads and writes, so they can be cutoff points
        ...
        // Condition 2: Found a READ or RMW on the same location - follow RF edges to find writes
        // RMW events are both reads and writes, so their read part can follow RF edges
        if (curr_event->get_location() == target_location &&
        ...
        // The mode checking is incorrect as sw edges are not only for acquire reads/fences/RMWs, 
        // but also for thread creation, joins where there could be different access modes and event types. 
