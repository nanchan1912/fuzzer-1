...
#include <assert.h>
#include "diversity_checker.h"  // for mo, rf coverage counts
...
struct queue_entry* mutate_run_enqueue_graph(afl_state_t* afl, struct queue_entry* parent) {
  ...
  if (new_score > afl->cutoff_score || afl->queued_items == 0 || (afl->check_data_race && mutated_graph_metadata->is_racy)) {
    /* Write graph */
...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (unlikely(afl->queue_cur && afl->queue_cur->graph_data && afl->queue_cur->graph_data->non_instantiable)) {
    ...
    reduce_queue_perf_score_hard(afl->queue_cur);
...
// End changes by us
  ...
  if (!splice_cycle) {
    ...
    afl->stage_name = "havoc_x";
    afl->stage_short = "havoc_x";
...
static u8 mopt_common_fuzzing(afl_state_t *afl, MOpt_globals_t MOpt_globals) {
  ...
  if (likely(!afl->old_seed_selection))
    ...
    orig_perf = perf_score = calculate_score(afl, afl->queue_cur->graph_data);
