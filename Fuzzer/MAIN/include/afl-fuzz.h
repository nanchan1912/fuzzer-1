...
// Begin changes by us
// Frameshift things not there now, are there in current afl
/************************
...
/*
Helper function to reduce the perf_score of a queue entry by 90% (or set to 1.0 if it is already low)
*/
static inline void reduce_queue_perf_score_hard(struct queue_entry *q) {
  if (!q) { return; }
  // if (q->perf_score > 1.0) {
  //   double decayed = q->perf_score * 0.1;
  //   if (decayed < 1.0) {
  //     decayed = 1.0;
  //   }
  //   q->perf_score = decayed;
  // } else {
  //   q->perf_score = 1.0;
  // }
  q->perf_score = 0;
...
typedef struct afl_state {
  ...
  // I have also defined this in forkserver.h: afl_formkserver struct
...
u8 save_if_interesting_skeleton(afl_state_t *, const struct SkeletonGraph *,
                                u8);
...
double  calculate_score(afl_state_t *, struct SkeletonGraphData *);
...
#endif
u8 save_if_interesting(afl_state_t *, void *, u32, u8, size_t, size_t);
