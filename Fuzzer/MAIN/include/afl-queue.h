/*
 * afl-queue.h -- Public API for bounded queue implementations.
 *
 ...
 *
 * All three implement the same interface; swap at runtime via AFL_QUEUE_IMPL env var.
 */
 ...
 *
 * Note: The queue takes ownership of graph_data pointer; caller must not
 * free it. Caller is responsible for cleaning up evicted entries if needed.
 */
 ...
 *
 * @param q           queue handle
 * @return            pointer to AflQueueEntry (stack allocated, valid until next op),
