...
#ifdef SGF_PERSISTENT_REPLAY_ARGPARSE
static char **record_arg = NULL;
#endif  // SGF_PERSISTENT_REPLAY_ARGPARSE
...
__attribute__((constructor)) static void __afl_record_replay_init(
   ...
   * the number of iterations defined for SGF_LOOP (on the same file)*/
  if (!(is_replay_record = !!getenv("SGF_PERSISTENT_REPLAY"))) {

    // printf("[warning] SGF_PERSISTENT_REPLAY not set.\n");
    return;

  }

  replay_record = atoi(getenv("SGF_PERSISTENT_REPLAY"));
  replay_record_dir = getenv("SGF_PERSISTENT_DIR");
