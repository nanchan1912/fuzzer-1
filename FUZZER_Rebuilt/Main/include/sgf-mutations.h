/* Implementation of sgf havoc mutation to be used in AFL++ custom mutators and
   partially in sgf-fuzz itself.

   How to use:

   #include "sgf-mutations.h"  // needs sgf-fuzz.h

   u32 afl_mutate(sgf_state_t *sgf, u8 *buf, u32 len, u32t steps, bool is_text,
   ...
   Parameters:
     sgf_state_t *sgf - the *sgf state pointer
...
#ifndef SGF_MUTATIONS_H
#define SGF_MUTATIONS_H
...
u32 afl_mutate(sgf_state_t *, u8 *, u32, u32, bool, bool, u8 *, u32, u32);
u32 choose_block_len(sgf_state_t *, u32);

/* Helper to choose random block len for block operations in fuzz_one().
   Doesn't return zero, provided that max_len is > 0. */

inline u32 choose_block_len(sgf_state_t *sgf, u32 limit) {

  u32 min_value, max_value;
  u32 rlim = MIN(sgf->queue_cycle, (u32)3);

  if (unlikely(!sgf->run_over10m)) { rlim = 1; }

  switch (rand_below(sgf, rlim)) {
  ...
  return min_value + rand_below(sgf, MIN(max_value, limit) - min_value + 1);

}

inline u32 afl_mutate(sgf_state_t *sgf, u8 *buf, u32 len, u32 steps,
  ...
  retry_havoc_step: {
    ...
    u32 r = rand_below(sgf, MUT_STRATEGY_ARRAY_SIZE), item;
    ...
    switch (mutation_array[r]) {
      ...
      case MUT_FLIPBIT: {
        ...
        u8  bit = rand_below(sgf, 8);
        u32 off = rand_below(sgf, len);
        buf[off] ^= 1 << bit;
      ...
      case MUT_INTERESTING8: {
        ...
        item = rand_below(sgf, sizeof(interesting_8));
        buf[rand_below(sgf, len)] = interesting_8[item];
        break;
      ...
      case MUT_INTERESTING16: {
        ...
        item = rand_below(sgf, sizeof(interesting_16) >> 1);
        INSERT16(buf, rand_below(sgf, len - 1), interesting_16[item]);
      ...
      case MUT_INTERESTING16BE: {
        ...
        item = rand_below(sgf, sizeof(interesting_16) >> 1);
        INSERT16(buf, rand_below(sgf, len - 1), SWAP16(interesting_16[item]));
      ...
      case MUT_INTERESTING32: {
        ...
        item = rand_below(sgf, sizeof(interesting_32) >> 2);
        INSERT32(buf, rand_below(sgf, len - 3), interesting_32[item]);
      ...
      case MUT_INTERESTING32BE: {
        ...
        item = rand_below(sgf, sizeof(interesting_32) >> 2);
        INSERT32(buf, rand_below(sgf, len - 3), SWAP32(interesting_32[item]));
      ...
      case MUT_ARITH8_: {
        ...
        item = 1 + rand_below(sgf, ARITH_MAX);
        buf[rand_below(sgf, len)] -= item;
        break;
      ...
      case MUT_RAND8: {
        ...
        u32 pos = rand_below(sgf, len);
        item = 1 + rand_below(sgf, 255);
        buf[pos] ^= item;
        break;
      ...
      case MUT_CLONE_COPY: {
        ...
        if (likely(len + HAVOC_BLK_XL < max_len)) {
          ...
          u32 clone_len = choose_block_len(sgf, len);
          u32 clone_from = rand_below(sgf, len - clone_len + 1);
          u32 clone_to = rand_below(sgf, len);
      ...
      case MUT_CLONE_FIXED: {
        ...
        if (likely(len + HAVOC_BLK_XL < max_len)) {
          ...
          u32 clone_len = choose_block_len(sgf, HAVOC_BLK_XL);
          u32 clone_to = rand_below(sgf, len);
          u32 strat = rand_below(sgf, 2);
          u32 clone_from = clone_to ? clone_to - 1 : 0;
          item = strat ? rand_below(sgf, 256) : buf[clone_from];
      ...
      case MUT_OVERWRITE_COPY: {
        ...
        u32 copy_len = choose_block_len(sgf, len - 1);
        u32 copy_from = rand_below(sgf, len - copy_len + 1);
        u32 copy_to = rand_below(sgf, len - copy_len + 1);
      ...
      case MUT_OVERWRITE_FIXED: {
        ...
        item = strat ? rand_below(sgf, 256) : buf[copy_from];
      ...
      case MUT_SWITCH: {
        ...
        switch_from = rand_below(sgf, len);
        do {

          switch_to = rand_below(sgf, len);
        ...
        switch_len = choose_block_len(sgf, MIN(switch_len, to_end));
      ...
      case MUT_DEL: {
        ...
        u32 del_len = choose_block_len(sgf, len - 1);
        u32 del_from = rand_below(sgf, len - del_len + 1);
      ...
      case MUT_SHUFFLE: {
        ...
        u32 blen = choose_block_len(sgf, len - 1);
        u32 off = rand_below(sgf, len - blen + 1);
      ...
      case MUT_ASCIINUM: {
        ...
        u32 off = rand_below(sgf, len), off2 = off, cnt = 0;
        ...
        switch (strat) {
          ...
          case 4:
            if (likely(val && (u64)val < 0x19999999)) {
              ...
              val = (u64)rand_next(sgf) % (u64)((u64)val * 10);

            } else {

              val = rand_below(sgf, 256);
      ...
      case MUT_INSERTASCIINUM: {
        ...
        u32 ins_len = 1 + rand_below(sgf, 8);
        u32 pos = rand_below(sgf, len);
      ...
      case MUT_EXTRA_OVERWRITE: {
        ...
        if (unlikely(!sgf->extras_cnt)) { goto retry_havoc_step; }

        /* Use the dictionary. */

        u32 use_extra = rand_below(sgf, sgf->extras_cnt);
        u32 extra_len = sgf->extras[use_extra].len;

        if (unlikely(extra_len > len)) { goto retry_havoc_step; }

        u32 insert_at = rand_below(sgf, len - extra_len + 1);
        memcpy(buf + insert_at, sgf->extras[use_extra].data, extra_len);
      ...
      case MUT_EXTRA_INSERT: {
        ...
        u8 *ptr = sgf->extras[use_extra].data;
        u32 insert_at = rand_below(sgf, len + 1);
      ...
      case MUT_AUTO_EXTRA_OVERWRITE: {
        ...
        if (unlikely(!sgf->a_extras_cnt)) { goto retry_havoc_step; }

        /* Use the dictionary. */

        u32 use_extra = rand_below(sgf, sgf->a_extras_cnt);
        u32 extra_len = sgf->a_extras[use_extra].len;

        if (unlikely(extra_len > len)) { goto retry_havoc_step; }

        u32 insert_at = rand_below(sgf, len - extra_len + 1);
        memcpy(buf + insert_at, sgf->a_extras[use_extra].data, extra_len);
      ...
      case MUT_AUTO_EXTRA_INSERT: {
        ...
        u8 *ptr = sgf->a_extras[use_extra].data;
        u32 insert_at = rand_below(sgf, len + 1);
      ...
      case MUT_SPLICE_OVERWRITE: {
        ...
        copy_len = choose_block_len(sgf, splice_len - 1);

        if (copy_len > len) copy_len = len;

        copy_from = rand_below(sgf, splice_len - copy_len + 1);
        copy_to = rand_below(sgf, len - copy_len + 1);
      ...
      case MUT_SPLICE_INSERT: {
        ...
        clone_len = choose_block_len(sgf, splice_len);
        clone_from = rand_below(sgf, splice_len - clone_len + 1);
        clone_to = rand_below(sgf, len + 1);
