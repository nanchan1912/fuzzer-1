...
// Begin changes by us in this file
// Exit codes by simulator
#define EXIT_EVENT_NOT_FOUND 10
#define EXIT_EVENT_MISMATCH 11
#define EXIT_RF_TYPE_MISMATCH 12
#define EXIT_DONE 0
#define EXIT_INSTANTIABLE 20
#define EXIT_NOT_INSTANTIABLE 21
// End changes by us in this file
...
#include "sgf-queue.h"              /* NEW: bounded queue system */
// End changes by us in this file
...
#define STAGE_BUF_SIZE (64)  /* usable size for stage name buf in sgf_state */

// Little helper to access the ptr to sgf->##name_buf - for use in afl_realloc.
#define SGF_BUF_PARAM(name) ((void **)&sgf->name##_buf)

#ifdef WORD_SIZE_64
  #define SGF_RAND_RETURN u64
#else
  #define SGF_RAND_RETURN u32
#endif
...
// Begin changes by us
// Frameshift things not there now, are there in current sgf
/************************
SKELETON GRAPH ABSTRACTION
***************************/

// Skeleton Graph is an abstraction over the execution graphs
// It is execution graph without values
#ifndef SKELETON_GRAPH_H
#define SKELETON_GRAPH_H

typedef struct SkeletonGraph SkeletonGraph; // this is opaque to C code

#endif


struct SkeletonGraphData{
    u32 id;

    // Graph data structure
    struct SkeletonGraph* skeleton_graph;

    // Data related to the execution by the simulator
    u8 sim_exit_code;                     /* Simulator exit code (non-crash)  */
    u8 already_simulated;                 /* Simulator already instantiated   */
    // void* simulator_feedback;             /* Opaque next-event feedback cache */
    ...
    void* skeleton_potential;             /* Opaque pointer to SkeletonPotential (C++ object) */
    u8    potential_indexed;              /* Added to NN index? */
    u8    potential_score_ready;          /* NN-based perf_score computed? */
    u64   potential_score_epoch;          /* NN epoch used for cached score */
    double potential_score;               /* Cached potential score */
...
struct queue_entry {
  ...
  // All the data related to skeleton graph for this entry
  struct SkeletonGraphData* graph_data;
...
typedef struct sgf_env_vars {

  u8 sgf_skip_cpufreq, sgf_exit_when_done, sgf_no_affinity, sgf_skip_bin_check,
      sgf_dumb_forksrv, sgf_import_first, sgf_custom_mutator_only,
      sgf_custom_mutator_late_send, sgf_no_ui, sgf_force_ui,
      sgf_i_dont_care_about_missing_crashes, sgf_bench_just_one,
      sgf_bench_until_crash, sgf_debug_child, sgf_autoresume, sgf_cal_fast,
      sgf_cycle_schedules, sgf_expand_havoc, sgf_statsd, sgf_cmplog_only_new,
      sgf_exit_on_seed_issues, sgf_try_affinity, sgf_ignore_problems,
      sgf_keep_timeouts, sgf_no_crash_readme, sgf_ignore_timeouts,
      sgf_no_startup_calibration, sgf_no_warn_instability,
      sgf_post_process_keep_original, sgf_crashing_seeds_as_new_crash,
      sgf_final_sync, sgf_ignore_seed_problems, sgf_disable_redundant,
  ...
  u8 *sgf_tmpdir, *sgf_custom_mutator_library, *sgf_python_module, *sgf_path,
      *sgf_hang_tmout, *sgf_forksrv_init_tmout, *sgf_preload,
      *sgf_max_det_extras, *sgf_statsd_host, *sgf_statsd_port,
      *sgf_crash_exitcode, *sgf_statsd_tags_flavor, *sgf_testcache_size,
      *sgf_testcache_entries, *sgf_child_kill_signal, *sgf_fsrv_kill_signal,
      *sgf_target_env, *sgf_persistent_record, *sgf_exit_on_time;

  s32 sgf_pizza_mode, sgf_ijon_history_limit;

  uid_t sgf_forksrv_uid;

  gid_t sgf_forksrv_gid;

  gid_t *sgf_forksrv_supl_gids;
...
typedef struct sgf_state {
  ...
  // setting the map size to 32 KB
  #define SKELETON_GRAPH_MAP_SIZE 32768

  u8 first_skeleton_graph; // to check if this is the first time we are executing the prgm

  // coverage corresponding to mo, rf
  u32 mo_coverage;
  u32 rf_coverage;
  ...
  u8 havoc_stack_pow2,                  /* HAVOC_STACK_POW2                 */
      ...
      autoresume,                       /* Resume if sgf->out_dir exists?   */
      ...
      old_seed_selection,               /* use vanilla sgf seed selection   */
...
#define N_FUZZ_SIZE_BITMAP (1 << 29)
  ...
  u64 total_crashes,                    /* Total number of crashes          */
      ...
      total_races,                      /* Total number of racy graphs      */
      saved_races,                      /* Racy graphs with unique pairs    */
      total_tmouts,                     /* Total number of timeouts         */
      saved_tmouts,                     /* Timeouts with unique signatures  */
      saved_hangs,                      /* Hangs with unique signatures     */
      last_crash_execs,                 /* Exec counter at last crash       */
      last_race_execs,                  /* Exec counter at last race        */
      ...
      last_race_time,                   /* Time for most recent race (ms)   */
  ...
  s32 sgf_ijon_history_limit;           /* IJON history buffer limit        */
  ...
  sgf_queue_t *bounded_queue;         /* NEW: the bounded queue system */
  ...
  char            *cmplog_binary;
  sgf_forkserver_t cmplog_fsrv;     /* cmplog has its own little forkserver */

  /* ASAN Fuzing */
  char            *san_binary[MAX_EXTRA_SAN_BINARY];
  sgf_forkserver_t san_fsrvs[MAX_EXTRA_SAN_BINARY];
...
#define FOREIGN_SYNCS_MAX 32U
  ...
  u32 plot_prev_mo_cov, plot_prev_rf_cov;
  ...
  // Begin changes by us
  u64 potential_nn_epoch;               /* NN cache epoch (increments on enqueue) */
  u32 potential_nn_recalc_interval;     /* Recompute NN after N new entries */
  ...
  u8 cutoff_percentile;                 /* Cutoff percentile for graphs score */
...
struct custom_mutator {
   ...
   *
   * @param sgf AFL instance.
   * @param seed Seed used for the mutation.
   * @return pointer to internal data or NULL on error
   */
  void *(*afl_custom_init)(sgf_state_t *sgf, unsigned int seed);

  /**
   * When sgf-fuzz was compiled with INTROSPECTION=1 then custom mutators can
   ...
   *
   * Empty dummy function. Its presence tells sgf-fuzz not to pass a
   ...
   * that you start the target with sgf-fuzz.
   *
   * (Optional)
   *
   * @param data pointer returned in afl_custom_init by this custom mutator
   ...
   * AFL++ executes the target with sgf-fuzz.
   *
   * (Optional)
   *
   * @param data pointer returned in afl_custom_init by this custom mutator
...
void afl_state_init(sgf_state_t *, uint32_t map_size);
void afl_state_deinit(sgf_state_t *);
void afl_resize_map_buffers(sgf_state_t *, u32 old_size, u32 new_size);
...
void read_afl_environment(sgf_state_t *, char **);

/**** Prototypes ****/

/* Custom mutators */
void setup_custom_mutators(sgf_state_t *);
void destroy_custom_mutators(sgf_state_t *);
u8   trim_case_custom(sgf_state_t *, struct queue_entry *q, u8 *in_buf,
                      struct custom_mutator *mutator);
void run_afl_custom_queue_new_entry(sgf_state_t *, struct queue_entry *, u8 *,
                                    u8 *);

/* Python */
#ifdef USE_PYTHON

struct custom_mutator *load_custom_mutator_py(sgf_state_t *, char *);
...
void  race_pair_store_update_incremental(void* store,
...
void mark_as_det_done(sgf_state_t *, struct queue_entry *);
void add_to_queue(sgf_state_t *, u8 *, u32, u8);
void destroy_queue(sgf_state_t *);
...
void write_bitmap(sgf_state_t *);
u32  count_bits(sgf_state_t *, u8 *);
u32  count_bytes(sgf_state_t *, u8 *);
u32  count_non_255_bytes(sgf_state_t *, u8 *);
void simplify_trace(sgf_state_t *, u8 *);
#ifdef WORD_SIZE_64
...
void minimize_bits(sgf_state_t *, u8 *, u8 *);
#ifndef SIMPLE_FILES
u8 *describe_op(sgf_state_t *, u8, size_t);
#endif
u8 save_if_interesting(sgf_state_t *, void *, u32, u8, size_t, size_t);
u8 has_new_bits(sgf_state_t *, u8 *);
#ifndef SGF_SHOWMAP
void classify_counts(sgf_forkserver_t *);
#endif

#ifdef __AFL_CODE_COVERAGE
void afl_pcmap_init(sgf_state_t *, u32);
void afl_pcmap_resize(sgf_state_t *, u32);
void afl_modmap_init(sgf_state_t *);
void afl_dump_pc_map(sgf_state_t *);
void afl_dump_module_map(sgf_state_t *);
#endif

/* Extras */

void load_extras_file(sgf_state_t *, u8 *, u32 *, u32 *, u32);
void load_extras(sgf_state_t *, u8 *);
void dedup_extras(sgf_state_t *);
void deunicode_extras(sgf_state_t *);
void add_extra(sgf_state_t *sgf, u8 *mem, u32 len);
void maybe_add_auto(sgf_state_t *, u8 *, u32);
void save_auto(sgf_state_t *);
void load_auto(sgf_state_t *);
void destroy_extras(sgf_state_t *);

/* Stats */

void load_stats_file(sgf_state_t *);
...
void show_stats_normal(sgf_state_t *);
void show_stats_pizza(sgf_state_t *);
void show_init_stats(sgf_state_t *);

void update_calibration_time(sgf_state_t *sgf, u64 *time);
void update_trim_time(sgf_state_t *sgf, u64 *time);
void update_sync_time(sgf_state_t *sgf, u64 *time);
void update_cmplog_time(sgf_state_t *sgf, u64 *time);
...
void check_sync_fuzzers(sgf_state_t *);
void sync_fuzzers(sgf_state_t *);
u32  write_to_testcase(sgf_state_t *, void **, u32, u32);
u8   calibrate_case(sgf_state_t *, struct queue_entry *, u8 *, u32, u8);
u8   trim_case(sgf_state_t *, struct queue_entry *, u8 *);
u8   common_fuzz_stuff(sgf_state_t *, u8 *, u32);
fsrv_run_result_t fuzz_run_target(sgf_state_t *, sgf_forkserver_t *fsrv, u32);

/* Fuzz one */

u8   fuzz_one_original(sgf_state_t *);
u8   pilot_fuzzing(sgf_state_t *);
u8   core_fuzzing(sgf_state_t *);
void pso_updating(sgf_state_t *);
u8   fuzz_one(sgf_state_t *);

/* Init */

#ifdef HAVE_AFFINITY
void bind_to_free_cpu(sgf_state_t *);
#endif
void   read_testcases(sgf_state_t *, u8 *);
void   perform_dry_run(sgf_state_t *);
void   pivot_inputs(sgf_state_t *);
u32    find_start_position(sgf_state_t *);
void   find_timeout(sgf_state_t *);
...
void   check_cpu_governor(sgf_state_t *);
void   get_core_count(sgf_state_t *);
void   fix_up_sync(sgf_state_t *);
void   check_asan_opts(sgf_state_t *);
void   check_binary(sgf_state_t *, u8 *);
u64    get_binary_hash(u8 *fn);
void   check_if_tty(sgf_state_t *);
void   save_cmdline(sgf_state_t *, u32, char **);
void   read_foreign_testcases(sgf_state_t *, int);
void   write_crash_readme(sgf_state_t *sgf);
...
u8 common_fuzz_cmplog_stuff(sgf_state_t *sgf, u8 *out_buf, u32 len);

/* RedQueen */
u8 input_to_state_stage(sgf_state_t *sgf, u8 *orig_buf, u8 *buf, u32 len);

/* our RNG wrapper */
SGF_RAND_RETURN rand_next(sgf_state_t *sgf);

/* probability between 0.0 and 1.0 */
double rand_next_percent(sgf_state_t *sgf);

/* SkipDet Functions */

u8 skip_deterministic_stage(sgf_state_t *, u8 *, u8 *, u32, u64);
u8 is_det_timeout(u64, u8);

void plot_profile_data(sgf_state_t *, struct queue_entry *);
...
static inline u32 rand_below(sgf_state_t *sgf, u32 limit) {

  if (unlikely(limit <= 1)) return 0;

  /* The boundary not being necessarily a power of 2,
     we need to ensure the result uniformity. */
  if (unlikely(!sgf->rand_cnt--) && likely(!sgf->fixed_seed)) {

    ck_read(sgf->fsrv.dev_urandom_fd, &sgf->rand_seed, sizeof(sgf->rand_seed),
            "/dev/urandom");
    sgf->rand_cnt = (RESEED_RNG / 2) + (sgf->rand_seed[1] % RESEED_RNG);
...
/* this is only called with normal havoc, not MOpt, to have an equalizer for
   expand havoc mode */
static inline u32 rand_below_datalen(sgf_state_t *sgf, u32 limit) {

  if (unlikely(limit <= 1)) return 0;

  switch (rand_below(sgf, 3)) {

    case 2:
      return (rand_below(sgf, limit) % (1 + rand_below(sgf, limit - 1))) %
             (1 + rand_below(sgf, limit - 1));
      break;
    case 1:
      return rand_below(sgf, limit) % (1 + rand_below(sgf, limit - 1));
      break;
    case 0:
      return rand_below(sgf, limit);
      break;
...
static inline s64 rand_get_seed(sgf_state_t *sgf) {

  if (unlikely(sgf->fixed_seed)) { return sgf->init_seed; }
  return sgf->rand_seed[0];

}

/* initialize randomness with a given seed. Can be called again at any time. */
void rand_set_seed(sgf_state_t *sgf, s64 init_seed);
...
u8 *queue_testcase_get(sgf_state_t *sgf, struct queue_entry *q);

/* If trimming changes the testcase size we have to reload it */
void queue_testcase_retake(sgf_state_t *sgf, struct queue_entry *q,
                           u32 old_len);

/* If trimming changes the testcase size we have to replace it  */
void queue_testcase_retake_mem(sgf_state_t *sgf, struct queue_entry *q, u8 *in,
                               u32 len, u32 old_len);

/* Add a new queue entry directly to the cache */

void queue_testcase_store_mem(sgf_state_t *sgf, struct queue_entry *q, u8 *mem);
...
/* Create file `fn`, but allow it to already exist if `SGF_SHA1_FILENAMES` is
 * enabled. */
static inline int permissive_create(sgf_state_t *sgf, const char *fn) {

  int fd = open(fn, O_WRONLY | O_CREAT | O_EXCL, sgf->perm);
  if (unlikely(fd < 0)) {

    if (!(sgf->sgf_env.sgf_sha1_filenames && errno == EEXIST)) {
