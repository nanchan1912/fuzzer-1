...
void update_cutoff(afl_state_t *afl, double current_cutoff, double new_score) {
  (void)current_cutoff;

  if (afl->queued_mu == 0.0) {
    afl->queued_mu = new_score;
    afl->queued_mad = new_score * 0.1;
  } else {
    afl->queued_mu = 0.95 * afl->queued_mu + 0.05 * new_score;
    double diff = new_score - afl->queued_mu;
    double abs_diff = diff < 0.0 ? -diff : diff;
    afl->queued_mad = 0.95 * afl->queued_mad + 0.05 * abs_diff;
  }

  double p = (double)afl->cutoff_percentile / 100.0;
  if (p <= 0.0) p = 0.01;
  if (p >= 1.0) p = 0.99;
  ...
  double sigma = 1.25 * afl->queued_mad;
  afl->cutoff_score = afl->queued_mu + z_score * sigma;

  if (afl->cutoff_score < 0.0) {
    afl->cutoff_score = 0.0;
  }
}
...
*/
struct queue_entry* mutate_run_enqueue_graph(afl_state_t* afl, struct queue_entry* parent) {
  ...
  /* Perform mutation */
  SkeletonGraph *working_graph = mutate_skeleton_graph_with_info(parent->graph_data->skeleton_graph, (int)afl->current_phase,
                                                            potential_obj,
                                                            (afl->enable_feedback && parent->graph_data) ?
                                                               parent->graph_data->simulator_feedback : NULL,
                                                             &mut_info, afl->enable_feedback);
  ...
  u8 exit_code = skeleton_graph_fuzz_stuff(afl, mutated_graph_metadata);
  ...
  /* Update Coverage */
  afl->mo_coverage = get_mo_coverage_count();
  afl->rf_coverage = get_rf_coverage_count();
  ...
  // Dynamic cutoff check: only add to queue if calculate_score(mutated_graph_metadata) > afl->cutoff_score
  double new_score = calculate_score(afl, mutated_graph_metadata);

  bool has_no_children = (parent && parent->graph_data && parent->graph_data->children_enqueued == 0);

  if (new_score > afl->cutoff_score || afl->queued_items == 0 || (afl->check_data_race && mutated_graph_metadata->is_racy) || has_no_children) {
    /* Write graph */
    char *filename = alloc_printf("%s/queue/id:%06u,src:%06u.json", afl->out_dir, afl->queued_items, parent->id);
    ...
    if (afl->queue_top) {
      ...
      if (afl->check_data_race && mutated_graph_metadata->is_racy) {
        save_race_if_interesting(afl, mutated_graph_metadata);
      }
      ...
      /* Index the fully-populated child in the bounded queue. This runs after
         ...
         the queue_entry and sets afl->queue_top, so `child` is only valid
      ...
      if (afl->bounded_queue) {
        if (afl_queue_enqueue(afl->bounded_queue, child->id, child, new_score) != 0) {
    ...
    update_cutoff(afl, afl->cutoff_score, new_score);
...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  // Begin changes by us
  if (unlikely(afl->queue_cur && afl->queue_cur->graph_data && afl->queue_cur->graph_data->non_instantiable)) {

    // ACTF("Skipping flagged entry %u (exit %u)", afl->queue_cur->id,
        //  afl->queue_cur->sim_exit_code);
  ...
  // Begin changes by us
  ACTF("Current entry %u has perf_score %0.2f", afl->queue_cur->id, afl->queue_cur->perf_score);
  ...
  if (afl->queue_cur->perf_score > 1.0) {
    double decayed = afl->queue_cur->perf_score * (1- afl->queue_cur->graph_data->decay_ratio);
    if (decayed < 1.0) { decayed = 1.0; }
    afl->queue_cur->perf_score = decayed;
  }
...
havoc_stage:
   ...
   ****************************/
    afl->stage_name = "skeleton graph fuzzing";
    afl->stage_short = "skeleton";
    const u32 highest_step = afl->skeleton_graph_stage_max ?
                                 afl->skeleton_graph_stage_max : 3;
    afl->stage_max = skeleton_graph_total_instances(highest_step);
    afl->stage_cur = 0;


    assert(afl -> queue_cur != NULL);

    // Have to run the seed to get feedback
    if(afl-> enable_feedback && afl->queue_cur->graph_data->already_simulated == 0){
      u8 exit_code = skeleton_graph_fuzz_stuff(afl, afl->queue_cur->graph_data);
    ...
    if(afl->enable_feedback){
      if(afl->queue_cur && afl->queue_cur->graph_data){
        if(!afl->queue_cur->graph_data->simulator_feedback){
    ...
    if (afl->queue_cur && afl->queue_cur->graph_data) {
      ...
      // exit(1);
      if (!afl->queue_cur->graph_data->skeleton_graph) {
        // No skeleton graph yet, read from JSON
        afl->queue_cur->graph_data->skeleton_graph = read_from_json(afl->queue_cur->fname);
        // If reading from JSON fails, create an empty skeleton graph
        if (!afl->queue_cur->graph_data->skeleton_graph) {
          afl->queue_cur->graph_data->skeleton_graph = empty_skeleton_graph();
        }
      }
      // If no potential object exists for the skeleton graph, create one
      if (!afl->queue_cur->graph_data->skeleton_potential) {
        afl->queue_cur->graph_data->skeleton_potential = create_skeleton_potential(afl->queue_cur->graph_data->skeleton_graph);
      }
    }
    ...
    prev_step_entries[0] = afl->queue_cur;
    prev_step_count = 1;

    for (u32 step = 1; step <= highest_step && afl->stage_cur < afl->stage_max; step++) {
      ...
      for (u32 instance = 0; instance < target_count && afl->stage_cur < afl->stage_max; ++instance) {
        ...
        if (step == 1) {
          ...
          if (instance < prev_step_count) {
            ...
            parent = prev_step_count > 0 ? prev_step_entries[instance % prev_step_count] : afl->queue_cur;
          }
        }

        struct queue_entry *child = mutate_run_enqueue_graph(afl, parent);
        if (!child) {
...
u8 fuzz_one(afl_state_t *afl) {
  ...
  // original -L flag parsing in afl-fuzz.c should be removed alongside this
