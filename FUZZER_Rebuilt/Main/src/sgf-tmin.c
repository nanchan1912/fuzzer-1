...
extern void destroy_custom_mutators(sgf_state_t *);
...
struct custom_mutator *load_custom_mutator(sgf_state_t *, const char *);
#ifdef USE_PYTHON
struct custom_mutator *load_custom_mutator_py(sgf_state_t *, char *);
#endif

static sgf_state_t *sgf;               /* State for custom mutators         */
...
#ifndef USE_PYTHON
struct custom_mutator *load_custom_mutator_py(sgf_state_t *sgf, char *module) {
...
static inline u8 anything_set(sgf_forkserver_t *fsrv) {
...
static void pre_afl_fsrv_write_to_testcase(sgf_forkserver_t *fsrv, u8 *mem,
                                           u32 len) {

  static u8 buf[MAX_FILE];
  u32       sent = 0;

  if (sgf && sgf->custom_mutators_count) {
...
static u8 tmin_run_target(sgf_forkserver_t *fsrv, u8 *mem, u32 len,
                          u8 first_run) {
...
static void minimize(sgf_forkserver_t *fsrv) {
  ...
  char *py_module = getenv("SGF_PYTHON_MODULE");
  if (py_module) {

    // We cannot use Python custom mutators in tmin
    if (debug) WARNF("Python custom mutator support not available in sgf-tmin");
    ...
    LIST_FOREACH(&sgf->custom_mutator_list, struct custom_mutator, {
      ...
      if (el->afl_custom_init_trim && el->afl_custom_trim &&
        ...
        while (cur_step < max_steps) {
          ...
          if (trimmed_size >= in_len && sgf->debug) {
...
static void set_up_environment(sgf_forkserver_t *fsrv, char **argv) {
  ...
  if (!out_file) {
    ...
    out_file = alloc_printf("%s/.sgf-tmin-temp-%u", use_dir, (u32)getpid());
    remove_out_file = 1;
...
static void usage(u8 *argv0) {
  ...
  SAYF(
      ...
      "SGF_TMIN_EXACT: require execution paths to match for crashing inputs\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  SAYF(cCYA "sgf-tmin" VERSION cRST " by Michal Zalewski\n");
    ...
    switch (opt) {
      ...
      case 'B':                                              /* load bitmap */
        ...
        /* This is a secret undocumented option! It is speculated to be useful
           ...
           sgf-showmap -b, and then load it into sgf-tmin via -B. The minimizer
  ...
  exact_mode = !!get_afl_env("SGF_TMIN_EXACT");

  if (hang_mode && exact_mode) {

    SAYF("SGF_TMIN_EXACT won't work for loops in hang mode, ignoring.");
    exact_mode = 0;
  ...
  if (sgf) {
    ...
    list_init(&sgf->custom_mutator_list);
    sgf->custom_mutators_count = 0;

    memcpy(&sgf->fsrv, fsrv, sizeof(sgf_forkserver_t));
