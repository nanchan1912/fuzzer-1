...
static bool quiet_mode,                /* Hide non-essential messages?      */
    ...
    cmin_mode,                         /* Generate output in sgf-cmin mode? */
...
/* Classify tuple counts. Instead of mapping to individual bits, as in
   sgf-fuzz.c, we map to more user-friendly numbers between 1 and 8. */
...
/* dummy functions */
u32 write_to_testcase(sgf_state_t *sgf, void **mem, u32 a, u32 b) {
...
void update_bitmap_score(sgf_state_t *sgf, struct queue_entry *q, bool x) {
...
fsrv_run_result_t fuzz_run_target(sgf_state_t *sgf, sgf_forkserver_t *fsrv,
                                  u32 i) {
...
void classify_counts(sgf_forkserver_t *fsrv) {
...
static sharedmem_t *deinit_shmem(sgf_forkserver_t *fsrv,
...
static void analyze_results(sgf_forkserver_t *fsrv) {
...
static u32 write_results_to_file(sgf_forkserver_t *fsrv, u8 *outfile) {

  s32 fd;
  u32 i, ret = 0;

  u8 cco = !!getenv("SGF_CMIN_CRASHES_ONLY"),
     caa = !!getenv("SGF_CMIN_ALLOW_ANY");
  ...
  if (cmin_mode && (run_timed_out() || (!caa && child_crashed != cco))) {
    ...
    if (strcmp(outfile, "-")) {
      ...
      // create empty file to prevent error messages in sgf-cmin
...
void pre_afl_fsrv_write_to_testcase(sgf_forkserver_t *fsrv, u8 *mem, u32 len) {
...
static void showmap_run_target_forkserver(sgf_forkserver_t *fsrv, u8 *mem,
                                          u32 len) {
  ...
  if (stop_soon) {
    ...
    SAYF(cRST cLRD "\n+++ sgf-showmap folder mode aborted by user +++\n" cRST);
    exit(1);
 ...
 * (passed as a buffer). */
static void showmap_run_target_nyx_mode(sgf_forkserver_t *fsrv) {
...
static void showmap_run_target(sgf_forkserver_t *fsrv, char **argv) {
...
static void set_up_environment(sgf_forkserver_t *fsrv, char **argv) {
...
static void show_banner(void) {
  ...
  SAYF(cCYA "sgf-showmap" VERSION cRST " by Michal Zalewski\n");
...
static void usage(u8 *argv0) {
  ...
  SAYF(
      ...
      "               (Not necessary, here for consistency with other sgf-* "
      "tools)\n"
      "  -X         - use Nyx mode\n"
#endif
      "\n"
      ...
      "SGF_CMIN_CRASHES_ONLY: (cmin_mode) only write tuples for crashing "
      "inputs\n"
      "SGF_CMIN_ALLOW_ANY: (cmin_mode) write tuples for crashing inputs also\n"
      "SGF_CRASH_EXITCODE: optional child exit code to be interpreted as "
      "crash\n"
      "SGF_DEBUG: enable extra developer output\n"
      "SGF_FORKSRV_INIT_TMOUT: time spent waiting for forkserver during "
      "startup (in milliseconds)\n"
      "SGF_KILL_SIGNAL: Signal ID delivered to child processes on timeout, "
      "etc.\n"
      "                 (default: SIGKILL)\n"
      "SGF_FORK_SERVER_KILL_SIGNAL: Kill signal for the fork server on "
      "termination\n"
      "                             (default: SIGTERM). If unset and "
      "SGF_KILL_SIGNAL is\n"
      "                             set, that value will be used.\n"
      "SGF_MAP_SIZE: the shared memory size for that target. must be >= the "
      "size the\n"
      "              target was compiled for\n"
      "SGF_PRELOAD: LD_PRELOAD / DYLD_INSERT_LIBRARIES settings for target\n"
      "SGF_PRINT_FILENAMES: Print the queue entry currently processed will to "
      "stdout\n"
      "SGF_QUIET: do not print extra informational output\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  sgf_forkserver_t fsrv_var = {0};
  if (getenv("SGF_DEBUG")) { debug = true; }
  if (get_afl_env("SGF_PRINT_FILENAMES")) { print_filenames = true; }
  ...
  if (getenv("SGF_QUIET") != NULL) { be_quiet = true; }
  ...
  while ((opt = getopt(argc, argv, "+i:I:o:f:m:t:AeqCZOH:QUWbcrshXY")) > 0) {
    ...
    switch (opt) {
      ...
      case 'f':  // only in here to avoid a compiler warning for use_stdin
        ...
        FATAL("Option -f is not supported in sgf-showmap");
      ...
      case 'Z':
        ...
        /* This is an undocumented option to write data in the syntax expected
           by sgf-cmin. Nobody else should have any use for this. */
      ...
      case 'H':
        /* Another sgf-cmin specific feature. */
        at_file = optarg;
        break;
      ...
      case 'O':                                               /* FRIDA mode */
        ...
        setenv("SGF_FRIDA_INST_SEED", "1", 1);
      ...
      case 'b':
        ...
        /* Secret undocumented mode. Writes output in raw binary format
           similar to that dumped by sgf-fuzz in <out_dir/queue/fuzz_bitmap. */
  ...
  if (getenv("SGF_NO_FORKSRV")) {             /* if set, use the fauxserver */
  ...
  if (in_dir || in_filelist) {
    ...
    stdin_file = at_file ? strdup(at_file)
                         : (char *)alloc_printf("%s/.sgf-showmap-temp-%u",
                                                use_dir, (u32)getpid());
    unlink(stdin_file);
  ...
  if (getenv("SGF_FORKSRV_INIT_TMOUT")) {

    s32 forksrv_init_tmout = atoi(getenv("SGF_FORKSRV_INIT_TMOUT"));
    if (forksrv_init_tmout < 1) {

      FATAL("Bad value specified for SGF_FORKSRV_INIT_TMOUT");
  ...
  if (!fsrv->cs_mode && !fsrv->qemu_mode && !unicorn_mode) {
    ...
    u32 new_map_size =
        afl_fsrv_get_mapsize(fsrv, use_argv, &stop_soon,
                             (get_afl_env("SGF_DEBUG_CHILD") ||
                              get_afl_env("SGF_DEBUG_CHILD_OUTPUT"))
                                 ? 1
                                 : 0);
  ...
  if (in_dir || in_filelist) {
    ...
    sgf->sgf_env.sgf_custom_mutator_library =
        getenv("SGF_CUSTOM_MUTATOR_LIBRARY");
    sgf->sgf_env.sgf_python_module = getenv("SGF_PYTHON_MODULE");
    setup_custom_mutators(sgf);

  } else {

    if (getenv("SGF_CUSTOM_MUTATOR_LIBRARY") || getenv("SGF_PYTHON_MODULE")) {
  ...
  if (in_dir || in_filelist) {
    ...
    if (getenv("SGF_DEBUG_GDB")) wait_for_gdb = true;
  ...
  if (cmin_mode && !!getenv("SGF_CMIN_CRASHES_ONLY")) {
