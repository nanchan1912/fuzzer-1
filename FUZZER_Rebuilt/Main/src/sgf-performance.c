...
void rand_set_seed(sgf_state_t *sgf, s64 init_seed) {
  ...
  sgf->init_seed = init_seed;
  sgf->rand_seed[0] =
      hash64((u8 *)&sgf->init_seed, sizeof(sgf->init_seed), HASH_CONST);
  sgf->rand_seed[1] = sgf->rand_seed[0] ^ 0x1234567890abcdef;
  sgf->rand_seed[2] = (sgf->rand_seed[0] & 0x1234567890abcdef) ^
                      (sgf->rand_seed[1] | 0xfedcba9876543210);
...
#ifdef WORD_SIZE_64
// romuDuoJr
inline SGF_RAND_RETURN rand_next(sgf_state_t *sgf) {

  SGF_RAND_RETURN xp = sgf->rand_seed[0];
  sgf->rand_seed[0] = 15241094284759029579u * sgf->rand_seed[1];
  sgf->rand_seed[1] = sgf->rand_seed[1] - xp;
  sgf->rand_seed[1] = ROTL(sgf->rand_seed[1], 27);
  return xp;
...
inline SGF_RAND_RETURN rand_next(sgf_state_t *sgf) {
  ...
  SGF_RAND_RETURN xp = sgf->rand_seed[0], yp = sgf->rand_seed[1],
                  zp = sgf->rand_seed[2];
  sgf->rand_seed[0] = 3323815723u * zp;
  sgf->rand_seed[1] = yp - xp;
  sgf->rand_seed[1] = ROTL(sgf->rand_seed[1], 6);
  sgf->rand_seed[2] = zp - yp;
  sgf->rand_seed[2] = ROTL(sgf->rand_seed[2], 22);
  return xp;
...
inline double rand_next_percent(sgf_state_t *sgf) {

  return (double)(((double)rand_next(sgf)) / (double)0xffffffffffffffff);

}

/* we switch from sgf's murmur implementation to xxh3 as it is 30% faster -
