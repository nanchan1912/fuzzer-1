...
static ijon_fastresume_state_t sgf_ijon_fastresume_state = {0};
static u8                      sgf_ijon_fastresume_loaded = 0;
...
void save_ijon_state_for_fastresume(u32 offset, u32 map_size, u32 real_map_size,
  ...
  sgf_ijon_fastresume_state.ijon_offset = offset;
  sgf_ijon_fastresume_state.map_size = map_size;
  sgf_ijon_fastresume_state.real_map_size = real_map_size;
  sgf_ijon_fastresume_state.target_map_size = target_map_size;
  sgf_ijon_fastresume_state.is_initialized = 1;
  sgf_ijon_fastresume_loaded = 1;

}

ijon_fastresume_state_t *get_saved_ijon_state(void) {

  return sgf_ijon_fastresume_loaded ? &sgf_ijon_fastresume_state : NULL;

}

u8 has_saved_ijon_state(void) {

  return sgf_ijon_fastresume_loaded;

}

void clear_saved_ijon_state(void) {

  memset(&sgf_ijon_fastresume_state, 0, sizeof(ijon_fastresume_state_t));
  sgf_ijon_fastresume_loaded = 0;
...
u32 get_saved_ijon_offset(void) {
  ...
  return sgf_ijon_fastresume_state.ijon_offset;
...
static void init_afl_ijon_history_limit(void) {
  ...
  if (sgf_ijon_history_limit_initialized) return;

  char *history_limit_env = getenv("SGF_IJON_HISTORY_LIMIT");
  sgf_ijon_history_limit_global =
      history_limit_env ? atoi(history_limit_env) : 0;
  sgf_ijon_history_limit_initialized = true;
...
void ijon_store_history_unconditional(ijon_min_state *self, int i,
  ...
  if (sgf_ijon_history_limit_global > 0) {
    ...
    if (variable_to_index[i] == -1) {
      ...
      if (num_discovered_vars + 1 > sgf_ijon_history_limit_global) {

        FATAL(
            "SGF_IJON_HISTORY_LIMIT=%d insufficient for %d variables. Minimum "
            "required: %d. "
            "Either increase limit or disable history (unset "
            "SGF_IJON_HISTORY_LIMIT).",
            sgf_ijon_history_limit_global, num_discovered_vars + 1,
    ...
    int idx = global_history_index++ % sgf_ijon_history_limit_global;

    char *history_filename = NULL;

    // Use global rolling buffer naming
    int padding = snprintf(NULL, 0, "%d", sgf_ijon_history_limit_global - 1);
