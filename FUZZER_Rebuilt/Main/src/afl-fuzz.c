...
int main(int argc, char **argv_orig, char **envp) {
  ...
  if (afl->bounded_queue) {
    ...
    for (entry = 0; entry < afl->queued_items; ++entry) {
      ...
      if (afl_queue_enqueue(afl->bounded_queue, q->id, q,
                            q->perf_score) == 0) {
        ++seeded;
      }
    ...
    ACTF("Bounded queue '%s': seeded %u/%u initial entries",
         afl->queue_impl_name ? afl->queue_impl_name : "?", seeded,
         valid_seeds);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      /* Bounded queue selection. This overrides the alias-table pick made
         ...
         AFL's own choice. */
      if (afl->bounded_queue && afl_queue_size(afl->bounded_queue) > 0) {

        AflQueueEntry *bq = afl_queue_dequeue(afl->bounded_queue);
        if (bq && bq->graph_data) {
