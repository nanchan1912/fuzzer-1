...
void afl_state_init(afl_state_t *afl, uint32_t map_size) {
  ...
  afl->potential_nn_epoch = 0;
  afl->potential_nn_recalc_interval = 64;  
  afl->skeleton_graph_stage_max = 3;
  afl->check_data_race = 0;
  afl->enable_feedback = 0;
  afl->cutoff_percentile = 1;

  afl->afl_env.skeleton_graph_stage_max = 3;
  afl->afl_env.check_data_race = 0;
  afl->afl_env.enable_feedback = 0;
  afl->afl_env.cutoff_percentile = 1;

#ifdef HAVE_AFFINITY
  afl->cpu_aff = -1;                    /* Selected CPU core                */
#endif                                                     /* HAVE_AFFINITY */

  afl->virgin_bits = ck_alloc(map_size + SKELETON_GRAPH_MAP_SIZE);
  afl->virgin_tmout = ck_alloc(map_size + SKELETON_GRAPH_MAP_SIZE);
  afl->virgin_crash = ck_alloc(map_size + SKELETON_GRAPH_MAP_SIZE);
  afl->var_bytes = ck_alloc(map_size + SKELETON_GRAPH_MAP_SIZE);
...
void afl_resize_map_buffers(afl_state_t *afl, u32 old_size, u32 new_size) {
  ...
  afl->virgin_bits = ck_realloc(afl->virgin_bits, new_size + SKELETON_GRAPH_MAP_SIZE);
  afl->virgin_tmout = ck_realloc(afl->virgin_tmout, new_size + SKELETON_GRAPH_MAP_SIZE);
  afl->virgin_crash = ck_realloc(afl->virgin_crash, new_size + SKELETON_GRAPH_MAP_SIZE);
  afl->var_bytes = ck_realloc(afl->var_bytes, new_size + SKELETON_GRAPH_MAP_SIZE);
    ...
    if (strncmp(env, "ALF_", 4) == 0) {
      ...
      while (match == 0 && afl_environment_variables[i] != NULL) {
        ...
        if (strncmp(env, afl_environment_variables[i],
          ...
          match = 1;
          if(!strncmp(env, "AFL_ENABLE_FEEDBACK", afl_environment_variable_len)){
            // 1. Actually fetch the variable
            afl->afl_env.enable_feedback = 
                atoi((u8 *)get_afl_env(afl_environment_variables[i]));

            // 2. Parse it if it exists
            if (afl->afl_env.enable_feedback) {
                afl->enable_feedback = afl->afl_env.enable_feedback;
            }
          ...
          } else if(!strncmp(env, "AFL_CHECK_DATA_RACE", afl_environment_variable_len)){
            // 1. Actually fetch the variable
            afl->afl_env.check_data_race = 
                atoi((u8 *)get_afl_env(afl_environment_variables[i]));

            // 2. Parse it if it exists
            if (afl->afl_env.check_data_race) {
                afl->check_data_race = afl->afl_env.check_data_race;
            }
          ...
          } else if (!strncmp(env, "AFL_SKELETON_GRAPH_HIGHEST_STEP", afl_environment_variable_len)) {
            
            // 1. Actually fetch the variable
            afl->afl_env.skeleton_graph_stage_max = 
                atoi((u8 *)get_afl_env(afl_environment_variables[i]));

            // 2. Parse it if it exists
            if (afl->afl_env.skeleton_graph_stage_max) {
                afl->skeleton_graph_stage_max = afl->afl_env.skeleton_graph_stage_max;
            }

            // 3. Fallback to default
            if (!afl->skeleton_graph_stage_max) {
                afl->skeleton_graph_stage_max = 1;
            }

          } else if(!strncmp(env, "AFL_CUTOFF_PERCENTILE", afl_environment_variable_len)){
            // 1. Actually fetch the variable
            afl->afl_env.cutoff_percentile = 
                atof((u8 *)get_afl_env(afl_environment_variables[i]));

            // 2. Parse it if it exists
            if (afl->afl_env.cutoff_percentile) {
                afl->cutoff_percentile = afl->afl_env.cutoff_percentile;
            }

            // 3. Fallback to default
            if (!afl->cutoff_percentile) {
                afl->cutoff_percentile = 1;
            }

          }
          else if (!strncmp(env, "AFL_SKIP_CPUFREQ", afl_environment_variable_len)) {
...
void afl_state_deinit(afl_state_t *afl) {
  ...
  if (afl->skel_hash_table) {
    ck_free(afl->skel_hash_table);
    afl->skel_hash_table = NULL;
    afl->skel_hash_cap = 0;
