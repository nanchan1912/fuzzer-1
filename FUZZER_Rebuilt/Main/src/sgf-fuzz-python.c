...
static void *unsupported(sgf_state_t *sgf, unsigned int seed) {
...
static py_mutator_t *init_py_module(sgf_state_t *sgf, u8 *module_name) {
  ...
  #endif
    if (py_functions[PY_FUNC_SPLICE_OPTOUT]) { sgf->custom_splice_optout = 1; }
...
static void init_py(sgf_state_t *sgf, py_mutator_t *py_mutator,
                    unsigned int seed) {
...
struct custom_mutator *load_custom_mutator_py(sgf_state_t *sgf,
                                              char        *module_name) {
  ...
  py_mutator = init_py_module(sgf, module_name);
  ...
  init_py(sgf, py_mutator, rand_below(sgf, 0xFFFFFFFF));
