...
public:
	...
	t_element * read_fetch() {
		unsigned int rdwr = atomic_load_explicit(&m_rdwr, memory_order_acquire);
		unsigned int rd,wr;
		for(;;) {
			rd = (rdwr>>16) & 0xFFFF;
			wr = rdwr & 0xFFFF;
	...
	void read_consume() {
		atomic_fetch_add_explicit(&m_read, 1, memory_order_release);
	}
	...
	t_element * write_prepare() {
		...
		for(;;) {
			...
			if (atomic_compare_exchange_weak_explicit(&m_rdwr, &rdwr, (rd << 16) | ((wr + 1) & 0xFFFF), memory_order_acq_rel, memory_order_acquire))
				break;
			else
				sched_yield();
		}
