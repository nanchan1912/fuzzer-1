...
void afl_nyx_runner_kill(sgf_forkserver_t *fsrv) {
...
static void fsrv_exec_child(sgf_forkserver_t *fsrv, char **argv) {

  if (fsrv->qemu_mode || fsrv->cs_mode) {

    setenv("SGF_DISABLE_LLVM_INSTRUMENTATION", "1", 0);
...
void afl_fsrv_init(sgf_forkserver_t *fsrv) {
  ...
  if (getenv("SGF_PRELOAD_DISCRIMINATE_FORKSERVER_PARENT") != NULL) {
...
void afl_fsrv_setup_preload(sgf_forkserver_t *fsrv, char *argv0) {
  ...
  u8 *sgf_preload = getenv("SGF_PRELOAD");
  u8 *preload_path = NULL;
  u8 *frida_binary = NULL;
  if (fsrv->frida_mode)
    frida_binary = find_afl_binary(argv0, "sgf-frida-trace.so");

  if (sgf_preload && frida_binary)
    preload_path = alloc_printf("%s:%s", sgf_preload, frida_binary);
  else if (sgf_preload)
    preload_path = ck_strdup(sgf_preload);
...
/* Internal forkserver for non_instrumented_mode=1 and non-forkserver mode runs.
  It execvs for each fork, forwarding exit codes and child pids to sgf. */

static void afl_fauxsrv_execv(sgf_forkserver_t *fsrv, char **argv) {
...
static void report_error_and_exit(int error) {
  ...
  switch (error) {
    ...
    case FS_ERROR_MAP_SIZE:
      FATAL(
          "SGF_MAP_SIZE is not set and fuzzing target reports that the "
          "required size is very large. Solution: Run the fuzzing target "
          "stand-alone with the environment variable SGF_DUMP_MAP_SIZE=1 set "
          "the displayed value in the SGF_MAP_SIZE environment variable for "
          "sgf-fuzz.");
      break;
    case FS_ERROR_MAP_ADDR:
      FATAL(
          ...
          "the target with either sgf-clang-lto and do not set "
          "SGF_LLVM_MAP_ADDR or recompile with sgf-clang-fast.");
      break;
...
/* Spins up fork server. The idea is explained here:
   ...
   through a pipe. The other part of this logic is in sgf-compilter-rt.o */
...
void afl_fsrv_start(sgf_forkserver_t *fsrv, char **argv,
  ...
  u32   status;
  s32   rlen;
  char *ignore_autodict = getenv("SGF_NO_AUTODICT");
  ...
  if (unlikely(fsrv->nyx_mode)) {
    ...
    char *nyx_log_path = getenv("SGF_NYX_LOG");
    if (nyx_log_path) {
      ...
      if (fsrv->nyx_log_fd < 0) {
        ...
        NYX_PRE_FATAL(fsrv, "SGF_NYX_LOG path could not be written");
    ...
    if (getenv("SGF_NYX_AUX_SIZE") != NULL) {

      fsrv->nyx_aux_string_len = atoi(getenv("SGF_NYX_AUX_SIZE"));

      if (fsrv->nyx_handlers->nyx_config_set_aux_buffer_size(
              nyx_config, fsrv->nyx_aux_string_len) != 1) {

        NYX_PRE_FATAL(fsrv,
                      "Invalid SGF_NYX_AUX_SIZE value set (must be a multiple "
                      "of 4096) ...");
    ...
    if (getenv("SGF_NYX_REUSE_SNAPSHOT") != NULL) {

      if (access(getenv("SGF_NYX_REUSE_SNAPSHOT"), F_OK) == -1) {

        NYX_PRE_FATAL(fsrv, "SGF_NYX_REUSE_SNAPSHOT path does not exist");
      ...
      char *snapshot_file_path =
          alloc_printf("%s/global.state", getenv("SGF_NYX_REUSE_SNAPSHOT"));
      if (access(snapshot_file_path, R_OK) == -1) {

        NYX_PRE_FATAL(fsrv,
                      "SGF_NYX_REUSE_SNAPSHOT path does not contain a valid "
                      "Nyx snapshot");
      ...
      char *reuse_snapshot_path_real =
          realpath(getenv("SGF_NYX_REUSE_SNAPSHOT"), NULL);

      if (strcmp(workdir_snapshot_path, reuse_snapshot_path_real) == 0) {

        NYX_PRE_FATAL(
            fsrv,
            "SGF_NYX_REUSE_SNAPSHOT path is located in current workdir "
      ...
      fsrv->nyx_handlers->nyx_config_set_reuse_snapshot_path(
          nyx_config, getenv("SGF_NYX_REUSE_SNAPSHOT"));
    ...
    fsrv->nyx_handlers->nyx_option_set_reload_mode(
        fsrv->nyx_runner, getenv("SGF_NYX_DISABLE_SNAPSHOT_MODE") == NULL);
      ...
      if (nyx_autodict_fd >= 0) {
        ...
        if (fstat(nyx_autodict_fd, &st) >= 0) {
          ...
          while (offset < (u32)f_len &&
            ...
            fsrv->add_extra_func(fsrv->sgf_ptr, dict + offset + 1,
                                 (u8)dict[offset]);
  ...
  if (!fsrv->fsrv_pid) {
    ...
    if (unlikely(fsrv->setenv)) { setenv("SGF_FORKSERVER_PARENT", "1", 0); }
      ...
      if ((status & FS_NEW_OPT_MAPSIZE)) {
        ...
        if (tmp_map_size > fsrv->map_size) {
          ...
          FATAL(
              ...
              "AFL++ is set with (%u). Either set SGF_MAP_SIZE=%u and "
              "restart "
              " sgf-fuzz, or change MAP_SIZE_POW2 in config.h and recompile "
              "sgf-fuzz",
      ...
      if ((status & FS_OPT_ENABLED) == FS_OPT_ENABLED) {
        ...
        if ((status & FS_OPT_AUTODICT) == FS_OPT_AUTODICT) {
          ...
          if (!ignore_autodict) {
            ...
            if (fsrv->add_extra_func == NULL || fsrv->sgf_ptr == NULL) {

              // this is not sgf-fuzz - or it is cmplog - we deny and return
        ...
        // if SGF_MAP_SIZE is set, use this map size
        if (getenv("SGF_MAP_SIZE") || getenv("SGF_MAPSIZE")) {
  ...
  if (fsrv->last_run_timed_out) {
    ...
    FATAL(
        ...
        "SGF_FORKSRV_INIT_TMOUT may help)");
  ...
  if (WIFSIGNALED(status)) {
    ...
    if (fsrv->mem_limit && fsrv->mem_limit < 500 && fsrv->uses_asan) {
      ...
      SAYF("\n" cLRD "[-] " cRST
           ...
           "conditions it needs\n"
           "      are not met. Try to:\n"
           "      1. Run again with SGF_DEBUG=1 set and check the output of "
           "the target\n"
           "         binary for clues.\n"
           "      2. Run again with SGF_DEBUG=1 and 'ulimit -c unlimited' and "
           "analyze the\n"
           "         generated core dump.\n\n"

           "    - Possibly the target requires a huge coverage map and has "
           "CTORS.\n"
           "      Retry with setting SGF_MAP_SIZE=10000000.\n\n"
  ...
  if (fsrv->mem_limit && fsrv->mem_limit < 500 && fsrv->uses_asan) {
    ...
    SAYF("\n" cLRD "[-] " cRST
         ...
         "it needs\n"
         "      are not met. Try to:\n"
         "      1. Run again with SGF_DEBUG=1 set and check the output of the "
         "target\n"
         "         binary for clues.\n"
         "      2. Run again with SGF_DEBUG=1 and 'ulimit -c unlimited' and "
         "analyze the\n"
    ...
    SAYF(
        ...
        "    - The target was compiled with sgf-clang-lto and a constructor "
        "was\n"
        "      instrumented, recompiling without SGF_LLVM_MAP_ADDR might solve "
        "your \n"
        "      problem\n\n"
...
void __attribute__((hot)) afl_fsrv_write_to_testcase(sgf_forkserver_t *fsrv,
                                                     u8 *buf, size_t len) {
...
fsrv_run_result_t __attribute__((hot)) afl_fsrv_run_target(
    sgf_forkserver_t *fsrv, u32 timeout, volatile u8 *stop_soon_p) {
  ...
  if (fsrv->nyx_mode) {
    ...
    switch (ret_val) {
      ...
      case InvalidWriteToPayload:
        if (!!getenv("SGF_NYX_HANDLE_INVALID_WRITE")) { return FSRV_RUN_CRASH; }
  ...
  abort_on_error. On top, a user may specify a custom SGF_CRASH_EXITCODE.
  ...
  for (i = 0; i < fsrv->persistent_record; ++i) {
    ...
    if (likely(len && data)) {
      ...
      snprintf(
          ...
          ((sgf_state_t *)(fsrv->sgf_ptr))->file_extension ? "." : "",
          ((sgf_state_t *)(fsrv->sgf_ptr))->file_extension
              ? (const char *)((sgf_state_t *)(fsrv->sgf_ptr))->file_extension
              : "");
...
void afl_fsrv_killall() {
  ...
  LIST_FOREACH(&fsrv_list, sgf_forkserver_t, {
