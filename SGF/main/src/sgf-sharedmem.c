...
u8 *afl_shm_init(sharedmem_t *shm, size_t map_size,
  ...
  snprintf(shm->g_shm_file_path, L_tmpnam, "/sgf_%d_%ld", getpid(), random());
  ...
  if (shm->cmplog_mode) {
    ...
    snprintf(shm->cmplog_g_shm_file_path, L_tmpnam, "/sgf_cmplog_%d_%ld",
             getpid(), random());
  ...
  if (shm->shm_id < 0) {
    ...
    PFATAL("shmget() failed, try running sgf-system-config");
