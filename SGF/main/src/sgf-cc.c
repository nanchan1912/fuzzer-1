...
#ifndef SGF_INCLUDE_PATH
  #define SGF_INCLUDE_PATH "/usr/local/include/sgf"
#endif
...
void aflcc_state_init(aflcc_state_t *aflcc, u8 *argv0) {
  ...
  if (getenv("SGF_DEBUG")) {
    ...
    aflcc->debug = 1;
    if (strcmp(getenv("SGF_DEBUG"), "0") == 0) unsetenv("SGF_DEBUG");

  } else if (getenv("SGF_QUIET")) {

    be_quiet = 1;

  }

  if (getenv("SGF_LLVM_NO_ERROR")) { aflcc->wnoerror = 1; }

  if ((getenv("SGF_PASSTHROUGH") || getenv("SGF_NOOPT")) && (!aflcc->debug)) {
...
/*
  ...
  1. firstly we check the $SGF_PATH environment variable location if set
  2. next we check argv[0] if it has path information and use it
    a) we also check ../lib/sgf
  3. if 2. failed we check /proc (only Linux, Android, NetBSD, DragonFly, and
     FreeBSD with procfs)
    a) and check here in ../lib/sgf too
  4. we look into the SGF_PATH define (usually /usr/local/lib/sgf)
...
u8 *find_object(aflcc_state_t *aflcc, u8 *obj) {
  ...
  u8 *sgf_path = getenv("SGF_PATH");
  u8 *slash = NULL, *tmp;

  if (sgf_path) {

    tmp = alloc_printf("%s/%s", sgf_path, obj);
  ...
  if (argv0) {
    ...
    if (slash) {
      ...
      *slash = 0;
      ...
      ck_free(tmp);
      tmp = alloc_printf("%s/../lib/sgf/%s", dir, obj);
        ...
        if (exepath_len > 0 && exepath_len < PATH_MAX) {
          ...
          if (slash) {
            ...
            *slash = 0;
            ...
            ck_free(tmp);
            tmp = alloc_printf("%s/../lib/sgf/%s", exepath, obj);
...
#undef HAS_PROC_FS
  ...
  if (strlen(obj) > 2 && obj[strlen(obj) - 1] == 'h' &&
    ...
    tmp = alloc_printf("%s/include/%s", sgf_path, obj);
    ...
    tmp = alloc_printf("%s/%s", SGF_INCLUDE_PATH, obj);
...
void find_built_deps(aflcc_state_t *aflcc) {
  ...
  if ((ptr = find_object(aflcc, "sgf-as")) != NULL) {
  ...
  if ((ptr = find_object(aflcc, "sgf-gcc-pass.so")) != NULL) {
  ...
  ptr = find_object(aflcc, "sgf-compiler-rt.o");

  if (!ptr) {

    FATAL(
        "Unable to find 'sgf-compiler-rt.o'. Please set the SGF_PATH "
...
/* Select compiler_mode by callname, such as "sgf-clang-fast", etc. */
void compiler_mode_by_callname(aflcc_state_t *aflcc) {

  if (strncmp(aflcc->callname, "sgf-clang-fast", 14) == 0) {

    /* sgf-clang-fast is always created there by makefile
    ...
    */
    ...
    aflcc->compiler_mode = CLANG;
      ...
      if (strncmp(aflcc->callname, "sgf-clang-lto", 13) == 0 ||

          strncmp(aflcc->callname, "sgf-lto", 7) == 0) {
    ...
    aflcc->compiler_mode = LTO;
      ...
      if (strncmp(aflcc->callname, "sgf-gcc-fast", 12) == 0 ||

          strncmp(aflcc->callname, "sgf-g++-fast", 12) == 0) {

    aflcc->compiler_mode = GCC_PLUGIN;

  } else if (strncmp(aflcc->callname, "sgf-gcc", 7) == 0 ||

             strncmp(aflcc->callname, "sgf-g++", 7) == 0) {

    aflcc->compiler_mode = GCC;

  } else if (strcmp(aflcc->callname, "sgf-clang") == 0 ||

             strcmp(aflcc->callname, "sgf-clang++") == 0) {
...
/*
  Select compiler_mode by env SGF_CC_COMPILER. And passthrough mode can be
  regarded as a special compiler_mode, so we check for it here, too.
*/
void compiler_mode_by_environ(aflcc_state_t *aflcc) {

  if (getenv("SGF_PASSTHROUGH") || getenv("SGF_NOOPT")) {

    aflcc->passthrough = 1;

  }

  char *ptr = getenv("SGF_CC_COMPILER");
  ...
  if (aflcc->compiler_mode) {
    ...
    if (!be_quiet) {
      ...
      WARNF(
          "\"SGF_CC_COMPILER\" is set but a specific compiler was already "
    ...
    if (strncasecmp(ptr, "LTO", 3) == 0) {
      ...
      FATAL("Unknown SGF_CC_COMPILER mode: %s\n", ptr);
...
void compiler_mode_by_cmdline(aflcc_state_t *aflcc, int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (strncmp(argv[i], "--sgf", 5) == 0) {

      if (!strcmp(argv[i], "--sgf_noopt") || !strcmp(argv[i], "--sgf-noopt")) {
      ...
      if (aflcc->compiler_mode && !be_quiet) {
        ...
        WARNF(
            "--sgf-... compiler mode supersedes the SGF_CC_COMPILER and "
      ...
      if (strncasecmp(ptr, "LTO", 3) == 0) {
        ...
        FATAL("Unknown --sgf-... compiler mode: %s\n", argv[i]);
...
/*
  ...
  - USE_TRACE_PC, SGF_USE_TRACE_PC, SGF_LLVM_USE_TRACE_PC, SGF_TRACE_PC
  - SGF_LLVM_CALLER, SGF_LLVM_CTX, SGF_LLVM_CTX_K
  - SGF_LLVM_NGRAM_SIZE
*/
static void instrument_mode_old_environ(aflcc_state_t *aflcc) {

  if (getenv("SGF_LLVM_INSTRIM") || getenv("INSTRIM") ||
      getenv("INSTRIM_LIB")) {
  ...
  if (getenv("USE_TRACE_PC") || getenv("SGF_USE_TRACE_PC") ||
      getenv("SGF_LLVM_USE_TRACE_PC") || getenv("SGF_TRACE_PC")) {

    if (aflcc->instrument_mode == 0)
      aflcc->instrument_mode = INSTRUMENT_PCGUARD;
    else if (aflcc->instrument_mode != INSTRUMENT_PCGUARD)
      FATAL("you cannot set SGF_LLVM_INSTRUMENT and SGF_TRACE_PC together");

  }

  if (getenv("SGF_LLVM_CTX")) aflcc->instrument_opt_mode |= INSTRUMENT_OPT_CTX;
  if (getenv("SGF_LLVM_CALLER") || getenv("SGF_LLVM_LTO_CALLER") ||
      getenv("SGF_LLVM_LTO_CTX"))
    aflcc->instrument_opt_mode |= INSTRUMENT_OPT_CALLER;

  if (getenv("SGF_LLVM_NGRAM_SIZE")) {

    aflcc->instrument_opt_mode |= INSTRUMENT_OPT_NGRAM;
    aflcc->ngram_size = atoi(getenv("SGF_LLVM_NGRAM_SIZE"));
  ...
  if (getenv("SGF_LLVM_CTX_K")) {

    aflcc->ctx_k = atoi(getenv("SGF_LLVM_CTX_K"));
    if (aflcc->ctx_k < 1 || aflcc->ctx_k > CTX_MAX_K)
      FATAL("K-CTX instrumentation mode must be between 1 and CTX_MAX_K (%u)",
            CTX_MAX_K);
    if (aflcc->ctx_k == 1) {

      setenv("SGF_LLVM_CALLER", "1", 1);
      unsetenv("SGF_LLVM_CTX_K");
      aflcc->instrument_opt_mode |= INSTRUMENT_OPT_CALLER;
...
/*
  Select instrument_mode by env 'SGF_LLVM_INSTRUMENT'.
...
static void instrument_mode_new_environ(aflcc_state_t *aflcc) {
  ...
  if ((ptr2 = getenv("SGF_OPT_LEVEL"))) {

    opt_level = ptr2[0];  // ignore invalid data

  }

  if (!getenv("SGF_LLVM_INSTRUMENT")) { return; }

  ptr2 = strtok(getenv("SGF_LLVM_INSTRUMENT"), ":,;");

  while (ptr2) {

    if (strncasecmp(ptr2, "sgf", strlen("sgf")) == 0 ||
    ...
    if (strncasecmp(ptr2, "ctx-", strlen("ctx-")) == 0 ||
      ...
      if (!*ptr3) {
        ...
        if ((ptr3 = getenv("SGF_LLVM_CTX_K")) == NULL)
          FATAL(
              "you must set the K-CTX K with (e.g. for value 2) "
              "SGF_LLVM_INSTRUMENT=ctx-2");
    ...
    if (strncasecmp(ptr2, "ngram", strlen("ngram")) == 0) {
      ...
      if (!*ptr3) {
        ...
        if ((ptr3 = getenv("SGF_LLVM_NGRAM_SIZE")) == NULL)
          FATAL(
              "you must set the NGRAM size with (e.g. for value 2) "
              "SGF_LLVM_INSTRUMENT=ngram-2");
      ...
      setenv("SGF_LLVM_NGRAM_SIZE", ptr4, 1);
...
void instrument_mode_by_environ(aflcc_state_t *aflcc) {
  ...
  if (getenv("SGF_LLVM_INSTRUMENT_FILE") || getenv("SGF_LLVM_WHITELIST") ||
      getenv("SGF_LLVM_ALLOWLIST") || getenv("SGF_LLVM_DENYLIST") ||
      getenv("SGF_LLVM_BLOCKLIST")) {

    aflcc->have_instr_env = 1;

  }

  if (aflcc->have_instr_env && getenv("SGF_DONT_OPTIMIZE") && !be_quiet) {

    WARNF(
        "SGF_LLVM_ALLOWLIST/DENYLIST and SGF_DONT_OPTIMIZE cannot be combined "
...
void mode_final_checkout(aflcc_state_t *aflcc, int argc, char **argv) {
  ...
  if (!aflcc->compiler_mode) {
    ...
    // lto is not a default because outside of sgf-cc RANLIB and AR have to
  ...
  if (aflcc->compiler_mode == CLANG) {
    ...
    if (!aflcc->have_optimized_pcguard &&
      ...
      aflcc->instrument_mode = INSTRUMENT_CLANG;
      setenv(CLANG_ENV_VAR, "1", 1);  // used by sgf-as
  ...
  if (aflcc->compiler_mode == LTO) {
    ...
    if (aflcc->instrument_mode == 0 ||
      ...
      if (!be_quiet) {
        ...
        WARNF("sgf-clang-lto called with mode %s, using that mode instead",
              instrument_mode_2str(aflcc->instrument_mode));
    ...
    if (aflcc->have_instr_env) {
      ...
      if (!be_quiet) {
        ...
        WARNF(
            ...
            "SGF_LLVM_ALLOWLIST/DENYLIST does not work with PCGUARD < 10.0.1.");
  ...
  if (aflcc->lto_mode) {
    ...
    if (aflcc->lto_flag[0] != '-')
      FATAL(
          "Using sgf-clang-lto is not possible because Makefile magic did not "
  ...
  if (getenv("SGF_LLVM_SKIP_NEVERZERO") && getenv("SGF_LLVM_NOT_ZERO"))
    FATAL(
        "SGF_LLVM_NOT_ZERO and SGF_LLVM_SKIP_NEVERZERO can not be set "
        "together");
  ...
  if (aflcc->instrument_mode == INSTRUMENT_PCGUARD && aflcc->have_instr_env) {
    ...
    FATAL(
        ...
        "SGF_LLVM_ALLOWLIST/DENYLIST! Use LLVM 10.0.1+ instead.");
  ...
  if ((ptr2 = getenv("SGF_LLVM_DICT2FILE")) != NULL && *ptr2 != '/')
    FATAL("SGF_LLVM_DICT2FILE must be set to an absolute file path");

  if (getenv("SGF_LLVM_LAF_ALL")) {

    setenv("SGF_LLVM_LAF_SPLIT_SWITCHES", "1", 1);
    setenv("SGF_LLVM_LAF_SPLIT_COMPARES", "1", 1);
    setenv("SGF_LLVM_LAF_SPLIT_FLOATS", "1", 1);
    setenv("SGF_LLVM_LAF_TRANSFORM_COMPARES", "1", 1);

  }

  if (getenv("SGF_LLVM_DICT2FILE") &&
      (getenv("SGF_LLVM_LAF_SPLIT_SWITCHES") ||
       getenv("SGF_LLVM_LAF_SPLIT_COMPARES") ||
       getenv("SGF_LLVM_LAF_SPLIT_FLOATS") ||
       getenv("SGF_LLVM_LAF_TRANSFORM_COMPARES")))
    FATAL("SGF_LLVM_DICT2FILE is incompatible with SGF_LLVM_LAF_*");

  aflcc->cmplog_mode = getenv("SGF_CMPLOG") || getenv("SGF_LLVM_CMPLOG") ||
                       getenv("SGF_GCC_CMPLOG");
...
void mode_notification(aflcc_state_t *aflcc) {
  ...
  if (!be_quiet &&
    ...
    WARNF(
        ...
        "gcc-plugin and use sgf-clang-fast/sgf-clang-lto/sgf-gcc-fast "
        "instead!");
...
/*
  ...
  - specified by env SGF_CXX
  - g++ or clang++
  - CLANGPP_BIN or LLVM_BINDIR/clang++
  when in C++ mode, or
  - specified by env SGF_CC
  - gcc or clang
  - CLANG_BIN or LLVM_BINDIR/clang
  otherwise.

  If SGF_COMPILER_LAUNCHER is set, prepend that command to the compiler.
*/
void add_real_argv0(aflcc_state_t *aflcc) {
  ...
  u8 *cc_prefix = getenv("SGF_COMPILER_LAUNCHER");
  if (cc_prefix) {
...
void add_defs_persistent_mode(aflcc_state_t *aflcc) {
  ...
  /* When the user tries to use persistent or deferred forkserver modes by
      ...
      signature into the binary (to be picked up by sgf-fuzz) and we want
...
#endif                                                        /* ^__APPLE__ */
      // if sgf is connected, we run _A times, else once.
...
/*
  ...
  - ASAN, MSAN, SGF_HARDEN are mutually exclusive.
  - Add options if not found there, on request of SGF_USE_ASAN, SGF_USE_MSAN,
  etc.
...
void add_sanitizers(aflcc_state_t *aflcc, char **envp) {
  ...
  if (getenv("SGF_USE_ASAN") || aflcc->have_asan) {

    if (getenv("SGF_USE_MSAN") || aflcc->have_msan)
      FATAL("ASAN and MSAN are mutually exclusive");

    if (getenv("SGF_HARDEN"))
      FATAL("ASAN and SGF_HARDEN are mutually exclusive");
  ...
  } else if (getenv("SGF_USE_MSAN") || aflcc->have_msan) {

    if (getenv("SGF_USE_ASAN") || aflcc->have_asan)
      FATAL("ASAN and MSAN are mutually exclusive");

    if (getenv("SGF_HARDEN"))
      FATAL("MSAN and SGF_HARDEN are mutually exclusive");
  ...
  if (getenv("SGF_USE_UBSAN") || aflcc->have_ubsan) {

    if (!aflcc->have_ubsan) { insert_param(aflcc, "-fsanitize=undefined"); }

    if (getenv("SGF_UBSAN_VERBOSE")) {
  ...
  if (getenv("SGF_USE_TSAN") || aflcc->have_tsan) {
  ...
  if (getenv("SGF_USE_LSAN") && !aflcc->have_lsan) {
  ...
  if (getenv("SGF_USE_RTSAN") && !aflcc->have_rtsan) {
  ...
  if (getenv("SGF_USE_CFISAN") || aflcc->have_cfisan) {
  ...
  if (aflcc->have_instr_list) {
    ...
    if (!be_quiet)
      SAYF(
          ...
          "SGF_LLVM_ALLOWLIST/SGF_LLVM_DENYLIST instead.\n");
...
param_st parse_linking_params(aflcc_state_t *aflcc, u8 *cur_argv, u8 scan,
  ...
  if (aflcc->lto_mode && !strncmp(cur_argv, "-flto=thin", 10)) {
    ...
    FATAL(
        "sgf-clang-lto cannot work with -flto=thin. Switch to -flto=full or "
        "use sgf-clang-fast!");
    ...
    if (scan) {
      ...
      if (aflcc->lto_mode)
        ...
        final_ = PARAM_KEEP;
             ...
             strstr(cur_argv, "sgf-compiler-rt") ||
             strstr(cur_argv, "sgf-llvm-rt")) {
...
void add_lto_linker(aflcc_state_t *aflcc) {
  ...
  unsetenv("SGF_LD");
  unsetenv("SGF_LD_CALLER");

  u8 *ld_path = NULL;
  if (getenv("SGF_REAL_LD")) {

    ld_path = strdup(getenv("SGF_REAL_LD"));

  } else {

    ld_path = strdup(SGF_REAL_LD);
...
#if defined(SGF_CLANG_LDPATH) && LLVM_MAJOR >= 12
    ...
    switch (aflcc->bit_mode) {
      ...
      case 0:
        if (!aflcc->shared_linking && !aflcc->partial_linking)
          insert_object(aflcc, "sgf-compiler-rt.o", 0, 0);
        if (aflcc->lto_mode) insert_object(aflcc, "sgf-llvm-rt-lto.o", 0, 0);
        break;

      case 32:
        if (!aflcc->shared_linking && !aflcc->partial_linking)
          insert_object(aflcc, "sgf-compiler-rt-32.o", 0, M32_ERR_MSG);
        if (aflcc->lto_mode)
          insert_object(aflcc, "sgf-llvm-rt-lto-32.o", 0, M32_ERR_MSG);
        break;

      case 64:
        if (!aflcc->shared_linking && !aflcc->partial_linking)
          insert_object(aflcc, "sgf-compiler-rt-64.o", 0, M64_ERR_MSG);
        if (aflcc->lto_mode)
          insert_object(aflcc, "sgf-llvm-rt-lto-64.o", 0, M64_ERR_MSG);
        break;

    }

  #if __AFL_CODE_COVERAGE
    // Required for dladdr used in sgf-compiler-rt.o
...
/*
  Add params to make compiler driver use our sgf-as
  as assembler, required by the vanilla instrumentation.
*/
void add_assembler(aflcc_state_t *aflcc) {

  u8 *sgf_as = find_object(aflcc, "sgf-as");

  if (!sgf_as) FATAL("Cannot find 'sgf-as'.");

  u8 *slash = strrchr(sgf_as, '/');
  if (slash) *slash = 0;

    // Search for 'as' may be unreliable in some cases (see #2058)
    // so use 'sgf-as' instead, because 'as' is usually a symbolic link,
    // or can be a renamed copy of 'sgf-as' created in the same dir.
    // Now we should verify if the compiler can find the 'as' we need.

#define SGF_AS_ERR "(should be a symlink or copy of 'sgf-as')"

  u8 *sgf_as_dup = alloc_printf("%s/as", sgf_as);

  int fd = open(sgf_as_dup, O_RDONLY);
  if (fd < 0) { PFATAL("Unable to open '%s' " SGF_AS_ERR, sgf_as_dup); }

  struct stat st;
  if (fstat(fd, &st) < 0) {

    PFATAL("Unable to fstat '%s' " SGF_AS_ERR, sgf_as_dup);
  ...
  if (f_data == MAP_FAILED) {
    ...
    PFATAL("Unable to mmap file '%s' " SGF_AS_ERR, sgf_as_dup);

  }

  close(fd);

  // "SGF_AS" is a const str passed to getenv in sgf-as.c
  if (!memmem(f_data, f_len, "SGF_AS", strlen("SGF_AS") + 1)) {

    FATAL(
        "Looks like '%s' is not a valid symlink or copy of '%s/sgf-as'. "
...
void add_gcc_plugin(aflcc_state_t *aflcc) {
  ...
  if (aflcc->cmplog_mode) {
    ...
    insert_object(aflcc, "sgf-gcc-cmplog-pass.so", "-fplugin=%s", 0);
    insert_object(aflcc, "sgf-gcc-cmptrs-pass.so", "-fplugin=%s", 0);

  }

  insert_object(aflcc, "sgf-gcc-pass.so", "-fplugin=%s", 0);
...
void add_misc_params(aflcc_state_t *aflcc) {
  ...
  if (getenv("SGF_NO_BUILTIN") || getenv("SGF_LLVM_LAF_TRANSFORM_COMPARES") ||
      getenv("SGF_LLVM_LAF_ALL") || getenv("SGF_LLVM_CMPLOG") ||
      aflcc->lto_mode) {
...
#define SCAN_KEEP(dst, src) \
  ...
  } else if (!strncmp(cur_argv, "--sgf", 5)) {
...
static void maybe_usage(aflcc_state_t *aflcc, int argc, char **argv) {
  ...
  if (argc < 2 || strncmp(argv[1], "-h", 2) == 0) {
    ...
    printf("sgf-cc" VERSION
           " by Michal Zalewski, Laszlo Szekeres, Marc Heuse\n");

    SAYF(
        "\n"
        "sgf-cc/sgf-c++ [options]\n"
        "\n"
        "This is a helper application for sgf-fuzz. It serves as a drop-in "
        "replacement\n"
        ...
        "  CC=sgf-cc CXX=sgf-c++ ./configure --disable-shared\n"
        "  cmake -DCMAKE_C_COMPILERC=sgf-cc -DCMAKE_CXX_COMPILER=sgf-c++ .\n"
        "  CC=sgf-cc CXX=sgf-c++ meson\n\n");
    ...
    SAYF(
        ...
        "sgf-clang-fast), set\n"
        "  the environment variable SGF_CC_COMPILER to a mode (e.g. LLVM) or "
        "use the\n"
        "  command line parameter --sgf-MODE (e.g. --sgf-llvm). If none is "
        "selected,\n"
        "  sgf-cc will select the best available (LLVM -> GCC_PLUGIN -> GCC).\n"
  ...
  #define NATIVE_MSG ""
    ...
    SAYF(
        "Sub-Modes: (set via env SGF_LLVM_INSTRUMENT, sgf-cc selects the best "
        "available)\n"
...
#undef NATIVE_MSG
    ...
    if (argc < 2 || strncmp(argv[1], "-hh", 3)) {
      ...
      SAYF(
          "To see all environment variables for the configuration of sgf-cc "
          "use \"-hh\".\n");
      ...
      SAYF(
          ...
          "  SGF_CC: path to the C compiler to use\n"
          "  SGF_CXX: path to the C++ compiler to use\n"
          "  SGF_DEBUG: enable developer debugging output\n"
          "  SGF_DONT_OPTIMIZE: disable optimization instead of -O3\n"
          "  SGF_NO_BUILTIN: no builtins for string compare functions (for "
          "libtokencap.so)\n"
          "  SGF_LLVM_NO_ERROR: strip out -Werror\n"
          "  SGF_NOOPT: behave like a normal compiler (to pass configure "
          "tests)\n"
          "  SGF_PATH: path to instrumenting pass and runtime  "
          "(sgf-compiler-rt.*o)\n"
          "  SGF_IGNORE_UNKNOWN_ENVS: don't warn on unknown env vars\n"
          "  SGF_INST_RATIO: percentage of branches to instrument\n"
          "  SGF_QUIET: suppress verbose output\n"
          "  SGF_HARDEN: adds code hardening to catch memory bugs\n"
          "  SGF_USE_ASAN: activate address sanitizer\n"
          "  SGF_USE_CFISAN: activate control flow sanitizer\n"
          "  SGF_USE_MSAN: activate memory sanitizer\n"
          "  SGF_USE_UBSAN: activate undefined behaviour sanitizer\n"
          "  SGF_USE_TSAN: activate thread sanitizer\n"
          "  SGF_USE_LSAN: activate leak-checker sanitizer\n"
          "  SGF_USE_RTSAN: activate realtime sanitizer\n"
          "  SGF_COMPILER_LAUNCHER: prepend command to compiler invocations "
          "(e.g., ccache)\n");

      if (aflcc->have_gcc_plugin)
        SAYF(
            "\nGCC Plugin-specific environment variables:\n"
            "  SGF_GCC_CMPLOG: log operands of comparisons (RedQueen mutator)\n"
            "  SGF_GCC_DISABLE_VERSION_CHECK: disable GCC plugin version "
            "control\n"
            "  SGF_GCC_OUT_OF_LINE: disable inlined instrumentation\n"
            "  SGF_GCC_SKIP_NEVERZERO: do not skip zero on trace counters\n"
            "  SGF_GCC_INSTRUMENT_FILE: enable selective instrumentation by "
            "filename\n");

#if LLVM_MAJOR >= 9
  #define COUNTER_BEHAVIOUR \
    "  SGF_LLVM_SKIP_NEVERZERO: do not skip zero on trace counters\n"
#else
  #define COUNTER_BEHAVIOUR \
    "  SGF_LLVM_NOT_ZERO: use cycling trace counters that skip zero\n"
#endif
      if (aflcc->have_llvm)
        SAYF(
            "\nLLVM/LTO/sgf-clang-fast/sgf-clang-lto specific environment "
            "variables:\n"
            "  SGF_LLVM_THREADSAFE_INST: instrument with thread safe counters, "
            "disables neverzero\n"

            COUNTER_BEHAVIOUR

            "  SGF_LLVM_DICT2FILE: generate an sgf dictionary based on found "
            "comparisons\n"
            "  SGF_LLVM_DICT2FILE_NO_MAIN: skip parsing main() for the "
            "dictionary\n"
            "  SGF_LLVM_INJECTIONS_ALL: enables all injections hooking\n"
            "  SGF_LLVM_INJECTIONS_SQL: enables SQL injections hooking\n"
            "  SGF_LLVM_INJECTIONS_LDAP: enables LDAP injections hooking\n"
            "  SGF_LLVM_INJECTIONS_XSS: enables XSS injections hooking\n"
            "  SGF_LLVM_IJON: enable IJON max tracking (auto-detects "
            "_USE_IJON)\n"
            "  SGF_LLVM_LAF_ALL: enables all LAF splits/transforms\n"
            "  SGF_LLVM_LAF_SPLIT_COMPARES: enable cascaded comparisons\n"
            "  SGF_LLVM_LAF_SPLIT_COMPARES_BITW: size limit (default 8)\n"
            "  SGF_LLVM_LAF_SPLIT_SWITCHES: cascaded comparisons on switches\n"
            "  SGF_LLVM_LAF_SPLIT_FLOATS: cascaded comparisons on floats\n"
            "  SGF_LLVM_LAF_TRANSFORM_COMPARES: cascade comparisons for string "
            "functions\n"
            "  SGF_LLVM_ALLOWLIST/SGF_LLVM_DENYLIST: enable "
            "instrument allow/\n"
            "    deny listing (selective instrumentation)\n");

      if (aflcc->have_llvm)
        SAYF(
            "  SGF_LLVM_CMPLOG: log operands of comparisons (RedQueen "
            "mutator)\n"
            "  SGF_LLVM_INSTRUMENT: set instrumentation mode:\n"
            "    CLASSIC, PCGUARD, LTO, GCC, CLANG, CALLER, CTX, NGRAM-2 "
            "..-16\n"
            " You can also use the old environment variables instead:\n"
            "  SGF_LLVM_USE_TRACE_PC: use LLVM trace-pc-guard instrumentation\n"
            "  SGF_LLVM_CALLER: use single context sensitive coverage (for "
            "CLASSIC)\n"
            "  SGF_LLVM_CTX: use full context sensitive coverage (for "
            "CLASSIC)\n"
            "  SGF_LLVM_NGRAM_SIZE: use ngram prev_loc count coverage (for "
            "CLASSIC)\n"
            "  SGF_LLVM_NO_RPATH: disable rpath setting for custom LLVM "
            "locations\n");

#ifdef SGF_CLANG_FLTO
      if (aflcc->have_lto)
        SAYF(
            "\nLTO/sgf-clang-lto specific environment variables:\n"
            "  SGF_LLVM_MAP_ADDR: use a fixed coverage map address (speed), "
            "e.g. "
            "0x10000\n"
            "  SGF_LLVM_DOCUMENT_IDS: write all edge IDs and the corresponding "
            "functions\n"
            "    into this file (LTO mode)\n"
            "  SGF_LLVM_LTO_CALLER: activate CALLER/CTX instrumentation\n"
            "  SGF_LLVM_LTO_CALLER_DEPTH: skip how many empty functions\n"
            "  SGF_LLVM_LTO_DONTWRITEID: don't write the highest ID used to a "
            "global var\n"
            "  SGF_LLVM_LTO_STARTID: from which ID to start counting from for "
            "a bb\n"
            "  SGF_REAL_LD: use this lld linker instead of the compiled in "
            "path\n"
            "  SGF_LLVM_LTO_SKIPINIT: don't inject initialization code "
            "(used in WAFL mode)\n"
            "If anything fails - be sure to read README.lto.md!\n");
#endif

      SAYF(
          "\nYou can supply --sgf-noopt to not instrument, like SGF_NOOPT. "
          "(this is helpful\n"
...
#if (LLVM_MAJOR >= 3)
    if (aflcc->have_lto)
      SAYF("sgf-cc LTO with ld=%s %s\n", SGF_REAL_LD, SGF_CLANG_FLTO);
    if (aflcc->have_llvm)
      SAYF("sgf-cc LLVM version %d using the binary path \"%s\".\n", LLVM_MAJOR,
           LLVM_BINDIR);
#endif
    ...
    SAYF(
        "Do not be overwhelmed :) sgf-cc uses good defaults if no options are "
        "selected.\n"
        "Read the documentation for FEATURES though, all are good but few are "
        "defaults.\n"
        "Recommended is sgf-clang-lto with SGF_LLVM_CMPLOG or sgf-clang-fast "
        "with\n"
        "SGF_LLVM_CMPLOG and "
        "SGF_LLVM_DICT2FILE+SGF_LLVM_DICT2FILE_NO_MAIN.\n\n");
...
/*
  Process params passed to sgf-cc.
...
static void edit_params(aflcc_state_t *aflcc, u32 argc, char **argv,
  ...
  if (aflcc->compiler_mode == LLVM || aflcc->compiler_mode == LTO) {
    ...
    if (aflcc->lto_mode && aflcc->have_instr_env) {
      ...
      load_llvm_pass(aflcc, "sgf-llvm-lto-instrumentlist.so");

    }

    if (getenv("SGF_LLVM_DICT2FILE")) {

      load_llvm_pass(aflcc, "sgf-llvm-dict2file.so");

    }

    // laf
    if (getenv("LAF_SPLIT_SWITCHES") || getenv("SGF_LLVM_LAF_SPLIT_SWITCHES")) {
    ...
    if (getenv("LAF_SPLIT_COMPARES") || getenv("SGF_LLVM_LAF_SPLIT_COMPARES") ||
        getenv("SGF_LLVM_LAF_SPLIT_FLOATS")) {
    ...
    if (aflcc->lto_mode) {
      ...
      if (aflcc->instrument_mode == INSTRUMENT_PCGUARD) {
        ...
        load_llvm_pass(aflcc, "sgf-llvm-pass.so");
    ...
    /* Load IJON instrumentation pass when SGF_LLVM_IJON is enabled */
    if (getenv("SGF_LLVM_IJON")) {

      load_llvm_pass(aflcc, "sgf-llvm-ijon-pass.so");
    ...
    if (getenv("SGF_LLVM_IJON")) {
      ...
      if (source_file && file_contains_ijon_usage(source_file)) {
        ...
        u8 *ijon_header = find_object(aflcc, "sgf-ijon-min.h");
        if (ijon_header) {
        ...
        if (getenv("SGF_DEBUG") && source_file) {
...
int main(int argc, char **argv, char **envp) {
  ...
  if (aflcc->instrument_mode == INSTRUMENT_GCC ||
    ...
    FATAL(
        "sgf-gcc/sgf-clang are obsolete and has been removed. Use "
        "sgf-clang-fast/sgf-gcc-fast for instrumentation instead.");
  ...
  if (aflcc->compiler_mode == LLVM) {
    ...
    if (getenv("SGF_LLVM_DICT2FILE")) {
      ...
      unsetenv("SGF_LLVM_DICT2FILE");

    }

    if (getenv("SGF_LLVM_LAF_SPLIT_SWITCHES") ||
        getenv("SGF_LLVM_LAF_SPLIT_COMPARES") ||
        getenv("SGF_LLVM_LAF_SPLIT_FLOATS") ||
        getenv("SGF_LLVM_LAF_TRANSFORM_COMPARES") ||
        getenv("SGF_LLVM_LAF_ALL")) {

      WARNF("SGF_LLVM_LAF support requires LLVM14+");
      unsetenv("SGF_LLVM_LAF_SPLIT_SWITCHES");
      unsetenv("SGF_LLVM_LAF_SPLIT_COMPARES");
      unsetenv("SGF_LLVM_LAF_SPLIT_FLOATS");
      unsetenv("SGF_LLVM_LAF_TRANSFORM_COMPARES");
      unsetenv("SGF_LLVM_LAF_ALL");
    ...
    if (getenv("SGF_LLVM_INJECTIONS_ALL") ||
      ...
      WARNF("SGF_LLVM_INJECTIONS support requires LLVM14+");
      unsetenv("SGF_LLVM_INJECTIONS_ALL");
      unsetenv("SGF_LLVM_INJECTIONS_SQL");
      unsetenv("SGF_LLVM_INJECTIONS_LDAP");
      unsetenv("SGF_LLVM_INJECTIONS_XSS");
