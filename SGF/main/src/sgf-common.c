...
#ifndef SGF_PATH
  #define SGF_PATH "/usr/local/lib/sgf/"
#endif
...
char **get_cs_argv(u8 *own_loc, u8 **target_path_p, int argc, char **argv) {
  ...
  if (unlikely(getenv("SGF_CS_CUSTOM_BIN"))) {

    WARNF(
        "SGF_CS_CUSTOM_BIN is enabled. "
        "You must run your target under sgf-cs-proxy on your own!");
    return argv;
  ...
  *target_path_p = new_argv[0] = find_afl_binary(own_loc, "sgf-cs-proxy");
  return new_argv;
...
char **get_qemu_argv(u8 *own_loc, u8 **target_path_p, int argc, char **argv) {
  ...
  if (unlikely(getenv("SGF_QEMU_CUSTOM_BIN"))) {

    WARNF(
        "SGF_QEMU_CUSTOM_BIN is enabled. "
        "You must run your target under sgf-qemu-trace on your own!");
    return argv;
  ...
  *target_path_p = new_argv[0] = find_afl_binary(own_loc, "sgf-qemu-trace");
  return new_argv;
...
char **get_wine_argv(u8 *own_loc, u8 **target_path_p, int argc, char **argv) {
  ...
  u8 *tmp = find_afl_binary(own_loc, "sgf-qemu-trace");
  ck_free(tmp);
  *target_path_p = new_argv[0] = find_afl_binary(own_loc, "sgf-wine-trace");
  return new_argv;
...
u8 *find_binary(u8 *fname) {
  ...
  // TODO: Merge this function with check_binary of sgf-fuzz-init.c
...
u8 *find_afl_binary(u8 *own_loc, u8 *fname) {
  ...
  u8 *sgf_path = NULL, *target_path, *own_copy, *tmp;
  int perm = X_OK;
  ...
  if ((sgf_path = getenv("SGF_PATH"))) {

    target_path = alloc_printf("%s/%s", sgf_path, fname);
...
void configure_afl_kill_signals(sgf_forkserver_t *fsrv,
  ...
  sgf_kill_signal_env =
      sgf_kill_signal_env ? sgf_kill_signal_env : getenv("SGF_KILL_SIGNAL");
  sgf_fsrv_kill_signal_env = sgf_fsrv_kill_signal_env
                                 ? sgf_fsrv_kill_signal_env
                                 : getenv("SGF_FORK_SERVER_KILL_SIGNAL");

  fsrv->child_kill_signal = parse_afl_kill_signal(sgf_kill_signal_env, SIGKILL);

  if (sgf_kill_signal_env && !sgf_fsrv_kill_signal_env) {

    /*
    Set SGF_FORK_SERVER_KILL_SIGNAL to the value of SGF_KILL_SIGNAL for
    backwards compatibility. However, if SGF_FORK_SERVER_KILL_SIGNAL is set, is
    takes precedence.
    */
    sgf_fsrv_kill_signal_env = sgf_kill_signal_env;

  }

  fsrv->fsrv_kill_signal = parse_afl_kill_signal(sgf_fsrv_kill_signal_env,
...
void print_suggested_envs(char *mispelled_env) {
  ...
  size_t env_name_len =
      strcspn(mispelled_env, "=") - 4;  // remove the SGF_prefix
  char *env_name = ck_alloc(env_name_len + 1);
  memcpy(env_name, mispelled_env + 4, env_name_len);

  char *seen = ck_alloc(sizeof(sgf_environment_variables) / sizeof(char *));
  int   found = 0;

  int j;
  for (j = 0; sgf_environment_variables[j] != NULL; ++j) {

    char *sgf_env = sgf_environment_variables[j] + 4;
    int   distance = string_distance_levenshtein(sgf_env, env_name);
    if (distance < ENV_SIMILARITY_TRESHOLD && seen[j] == 0) {

      SAYF("Did you mean %s?\n", sgf_environment_variables[j]);
      seen[j] = 1;
      found = 1;
  ...
  for (j = 0; sgf_environment_variables[j] != NULL; ++j) {
    ...
    char  *sgf_env = sgf_environment_variables[j] + 4;
    size_t sgf_env_len = strlen(sgf_env);
    char  *reduced = ck_alloc(sgf_env_len + 1);

    size_t start = 0;
    while (start < sgf_env_len) {

      size_t end = start + strcspn(sgf_env + start, "_") + 1;
      memcpy(reduced, sgf_env, start);
      if (end < sgf_env_len) {

        memcpy(reduced + start, sgf_env + end, sgf_env_len - end);

      }

      if (sgf_env_len + start >= end) {

        reduced[sgf_env_len - end + start] = 0;
  ...
  while (start < env_name_len) {
    ...
    for (j = 0; sgf_environment_variables[j] != NULL; ++j) {
      ...
      int distance = string_distance_levenshtein(
          sgf_environment_variables[j] + 4, reduced);
      if (distance < ENV_SIMILARITY_TRESHOLD && seen[j] == 0) {
...
void check_environment_vars(char **envp) {
  ...
  char *env, *val, *ignore = getenv("SGF_IGNORE_UNKNOWN_ENVS");
    ...
    if (strncmp(env, "ALF_", 4) == 0 || strncmp(env, "_ALF", 4) == 0 ||
      ...
      while (match == 0 && sgf_environment_variables[i] != NULL) {
        ...
        if (strncmp(env, sgf_environment_variables[i],
          ...
          if ((val = getenv(sgf_environment_variables[i])) && !*val) {

            WARNF(
                "AFL environment variable %s defined but is empty, this can "
                "lead to unexpected consequences",
                sgf_environment_variables[i]);
            issue_detected = 1;
      ...
      while (match == 0 && sgf_environment_deprecated[i] != NULL) {
        ...
        if (strncmp(env, sgf_environment_deprecated[i],
          ...
          if (strncmp(sgf_environment_deprecated[i], "SGF_SAN_NO_INST",
                      strlen(sgf_environment_deprecated[i])) == 0) {

            WARNF(
                "SGF_LLVM_ONLY_FSRV/SGF_GCC_ONLY_FSRV is induced and set "
                "instead.");
            setenv("SGF_GCC_ONLY_FSRV", "1", 0);
            setenv("SGF_LLVM_ONLY_FSRV", "1", 0);
...
u32 validate_map_size(u32 map_size) {
  ...
  if (!map_size || map_size >= (1U << 29)) {
    ...
    FATAL("illegal SGF_MAP_SIZE %u, must be between 64 <= %u < %u", map_size,
          64U, 1U << 29);
...
u32 get_map_size(void) {
  ...
  if ((ptr = getenv("SGF_MAP_SIZE")) || (ptr = getenv("SGF_MAPSIZE"))) {
  ...
  } else if (getenv("SGF_SKIP_BIN_CHECK")) {
