...
void setup_custom_mutators(sgf_state_t *sgf) {

  /* Try mutator library first */
  struct custom_mutator *mutator;
  u8                    *fn = sgf->sgf_env.sgf_custom_mutator_library;
  u32                    prev_mutator_count = 0;

  if (fn) {

    if (sgf->limit_time_sig && sgf->limit_time_sig != -1)
      FATAL(
    ...
    if (likely(!fn_token)) {
      ...
      mutator = load_custom_mutator(sgf, fn);
      list_append(&sgf->custom_mutator_list, mutator);
      sgf->custom_mutators_count++;
      ...
      while (fn_token) {
        ...
        if (*fn_token) {  // strsep can be empty if ";;"
          ...
          if (sgf->not_on_tty && sgf->debug)
            SAYF("[Custom] Processing: %s\n", fn_token);
          prev_mutator_count = sgf->custom_mutators_count;
          mutator = load_custom_mutator(sgf, fn_token);
          list_append(&sgf->custom_mutator_list, mutator);
          sgf->custom_mutators_count++;
          if (prev_mutator_count > sgf->custom_mutators_count)
  ...
  /* Try Python module */
#ifdef USE_PYTHON
  u8 *module_name = sgf->sgf_env.sgf_python_module;

  if (module_name) {

    if (sgf->limit_time_sig) {
    ...
    struct custom_mutator *m = load_custom_mutator_py(sgf, module_name);
    sgf->custom_mutators_count++;
    list_append(&sgf->custom_mutator_list, m);

  }

#else
  if (sgf->sgf_env.sgf_python_module) {
...
void destroy_custom_mutators(sgf_state_t *sgf) {

  if (sgf->custom_mutators_count) {

    LIST_FOREACH_CLEAR(&sgf->custom_mutator_list, struct custom_mutator, {
...
struct custom_mutator *load_custom_mutator(sgf_state_t *sgf, const char *fn) {
  ...
  if (mutator->afl_custom_init) {
    ...
    mutator->data = mutator->afl_custom_init(sgf, rand_below(sgf, 0xFFFFFFFF));
...
u8 trim_case_custom(sgf_state_t *sgf, struct queue_entry *q, u8 *in_buf,
                    struct custom_mutator *mutator) {
  ...
  while (sgf->stage_cur < sgf->stage_max) {

    u8 *retbuf = NULL;

    sprintf(sgf->stage_name_buf, "ptrim %s",
    ...
    if (likely(retlen)) {
      ...
      retlen = write_to_testcase(sgf, (void **)&retbuf, retlen, 0);
    ...
    if (likely(retlen && cksum == q->exec_cksum)) {
      ...
      if (!out_buf) {
        ...
        memcpy(sgf->clean_trace_custom, sgf->fsrv.trace_bits,
               sgf->fsrv.map_size);
      ...
      sgf->stage_cur = mutator->afl_custom_post_trim(mutator->data, 1);

      if (sgf->not_on_tty && sgf->debug) {

        DEBUGF(
            "[Custom Trimming] SUCCESS: %u/%u iterations (now at %u bytes)\n",
            sgf->stage_cur, sgf->stage_max, out_len);
  ...
  if (out_buf) {
    ...
    memcpy(sgf->fsrv.trace_bits, sgf->clean_trace_custom, sgf->fsrv.map_size);
  ...
  if (sgf->not_on_tty && sgf->debug) {
    ...
    OKF("[Custom Trimming] DONE: %u bytes -> %u bytes", orig_len, q->len);
