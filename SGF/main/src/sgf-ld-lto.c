...
static u8 *sgf_path = SGF_PATH;
static u8 *real_ld = SGF_REAL_LD;

static u8 be_quiet,                 /* Quiet mode (no stderr output)        */
    debug,                          /* SGF_DEBUG                            */
    passthrough,                    /* SGF_LD_PASSTHROUGH - no link+optimize*/
...
static void edit_params(int argc, char **argv) {
  ...
  if (!passthrough) {
    ...
    for (i = 1; i < (u32)argc; i++) {
      ...
      if (strstr(argv[i], "/sgf-llvm-rt-lto.o") != NULL) rt_lto_present = 1;
      if (strstr(argv[i], "/sgf-compiler-rt.o") != NULL) rt_present = 1;
      if (strstr(argv[i], "/sgf-llvm-lto-instr") != NULL) inst_present = 1;
    ...
    if (i == gold_pos && !passthrough) {
      ...
      if (!inst_present) {
        ...
        ld_params[ld_param_cnt++] = alloc_printf(
            "-mllvm=-load=%s/sgf-llvm-lto-instrumentation.so", sgf_path);

      }

      if (!rt_present)
        ld_params[ld_param_cnt++] =
            alloc_printf("%s/sgf-compiler-rt.o", sgf_path);
      if (!rt_lto_present)
        ld_params[ld_param_cnt++] =
            alloc_printf("%s/sgf-llvm-rt-lto.o", sgf_path);
...
int main(int argc, char **argv) {
  ...
  if (getenv("SGF_LD_CALLER") != NULL) {

    FATAL("ld loop detected! Set SGF_REAL_LD!\n");

  }

  if (isatty(2) && !getenv("SGF_QUIET") && !getenv("SGF_DEBUG")) {

    SAYF(cCYA "sgf-ld-to" VERSION cRST
  ...
  if (getenv("SGF_DEBUG") != NULL) debug = 1;
  if (getenv("SGF_PATH") != NULL) sgf_path = getenv("SGF_PATH");
  if (getenv("SGF_LD_PASSTHROUGH") != NULL) passthrough = 1;
  if (getenv("SGF_REAL_LD") != NULL) real_ld = getenv("SGF_REAL_LD");

  if (!sgf_path || !*sgf_path) sgf_path = SGF_PATH;

  setenv("SGF_LD_CALLER", "1", 1);
  ...
  if (argc < 2) {
    ...
    SAYF(
        "\n"
        "This is a helper application for sgf-clang-lto.\n"
        "It is a wrapper around llvm's 'lld' in case sgf-clang-lto cannot be "
        "used.\n"
        ...
        "  SGF_LD_PASSTHROUGH   do not link+optimize == no instrumentation\n"
        "  SGF_REAL_LD          point to the real llvm 11 lld if necessary\n"
