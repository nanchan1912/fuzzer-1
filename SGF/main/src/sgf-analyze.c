...
static sgf_forkserver_t fsrv = {0};   /* The forkserver                     */
...
static void set_up_environment(char **argv) {
  ...
  if (!fsrv.out_file) {
    ...
    fsrv.out_file =
        alloc_printf("%s/.sgf-analyze-temp-%u", use_dir, (u32)getpid());
...
static void usage(u8 *argv0) {
  ...
  SAYF(
      ...
      "SGF_ANALYZE_HEX: print file offsets in hexadecimal instead of decimal\n"
      ...
      "SGF_SKIP_BIN_CHECK: skip checking the location of and the target\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  SAYF(cCYA "sgf-analyze" VERSION cRST " by Michal Zalewski\n");
  ...
  use_hex_offsets = !!get_afl_env("SGF_ANALYZE_HEX");
  ...
  if (get_afl_env("SGF_SKIP_BIN_CHECK") == NULL && !anything_set()) {
