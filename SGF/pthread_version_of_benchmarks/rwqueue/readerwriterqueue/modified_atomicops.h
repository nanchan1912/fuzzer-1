...
AE_FORCEINLINE void compiler_fence(memory_order order) AE_NO_TSAN
{
	switch (order) {
		case memory_order_relaxed: break;
		case memory_order_acquire: atomic_signal_fence(memory_order_acquire); break;
		case memory_order_release: atomic_signal_fence(memory_order_release); break;
		case memory_order_acq_rel: atomic_signal_fence(memory_order_acq_rel); break;
		case memory_order_seq_cst: atomic_signal_fence(memory_order_seq_cst); break;
		default: assert(false);
	}
}
...
AE_FORCEINLINE void fence(memory_order order) AE_NO_TSAN
{
	switch (order) {
		case memory_order_relaxed: break;
		case memory_order_acquire: AE_TSAN_ANNOTATE_ACQUIRE(); atomic_thread_fence(memory_order_acquire); break;
		case memory_order_release: AE_TSAN_ANNOTATE_RELEASE(); atomic_thread_fence(memory_order_release); break;
		case memory_order_acq_rel: AE_TSAN_ANNOTATE_ACQUIRE(); AE_TSAN_ANNOTATE_RELEASE(); atomic_thread_fence(memory_order_acq_rel); break;
		case memory_order_seq_cst: AE_TSAN_ANNOTATE_ACQUIRE(); AE_TSAN_ANNOTATE_RELEASE(); atomic_thread_fence(memory_order_seq_cst); break;
		default: assert(false);
	}
}
...
#error Unsupported platform
	...
	{
		atomic_store_explicit(&value, std::forward<U>(x), memory_order_relaxed);
		return *this;
	}
	
	AE_FORCEINLINE weak_atomic const& operator=(weak_atomic const& other) AE_NO_TSAN
	{
		atomic_store_explicit(&value, atomic_load_explicit(&other.value, memory_order_relaxed), memory_order_relaxed);
		return *this;
	}

	AE_FORCEINLINE T load() const AE_NO_TSAN { return atomic_load_explicit(&value, memory_order_relaxed); }
	
	AE_FORCEINLINE T fetch_add_acquire(T increment) AE_NO_TSAN
	{
		return atomic_fetch_add_explicit(&value, increment, memory_order_acquire);
	}
	
	AE_FORCEINLINE T fetch_add_release(T increment) AE_NO_TSAN
	{
		return atomic_fetch_add_explicit(&value, increment, memory_order_release);
	}
#endif
