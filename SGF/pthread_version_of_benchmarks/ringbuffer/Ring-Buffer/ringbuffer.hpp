...
namespace jnk0le
{
	...
	template<typename T, size_t buffer_size = 16, bool fake_tso = false, size_t cacheline_size = 0, typename index_t = size_t>
		...
		public:
			...
			void consumerClear(void) {
				atomic_store_explicit(&tail, atomic_load_explicit(&head, memory_order_relaxed), memory_order_relaxed);
			}
			...
			index_t readAvailable(void) const {
				return atomic_load_explicit(&head, index_acquire_barrier) - atomic_load_explicit(&tail, memory_order_relaxed);
			}
			...
			index_t writeAvailable(void) const {
				return buffer_size - (atomic_load_explicit(&head, memory_order_relaxed) - atomic_load_explicit(&tail, index_acquire_barrier));
			}
			...
			bool insert(T data)
			{
				index_t tmp_head = atomic_load_explicit(&head, memory_order_relaxed);

				if((tmp_head - atomic_load_explicit(&tail, index_acquire_barrier)) == buffer_size)
					return false;
				else
				{
					data_buff[tmp_head++ & buffer_mask] = data;
					atomic_signal_fence(memory_order_release);
					atomic_store_explicit(&head, tmp_head, index_release_barrier);
				}
				return true;
			}
			...
			bool remove()
			{
				index_t tmp_tail = atomic_load_explicit(&tail, memory_order_relaxed);

				if(tmp_tail == atomic_load_explicit(&head, memory_order_relaxed))
					return false;
				else
					atomic_store_explicit(&tail, ++tmp_tail, index_release_barrier); // release in case data was loaded/used before
			...
			size_t remove(size_t cnt) {
				index_t tmp_tail = atomic_load_explicit(&tail, memory_order_relaxed);
				index_t avail = atomic_load_explicit(&head, memory_order_relaxed) - tmp_tail;

				cnt = (cnt > avail) ? avail : cnt;

				atomic_store_explicit(&tail, tmp_tail + cnt, index_release_barrier);
				return cnt;
			}
			...
			bool remove(T* data) {
				...
				if(tmp_tail == atomic_load_explicit(&head, index_acquire_barrier))
					return false;
				else
				{
					*data = data_buff[tmp_tail++ & buffer_mask];
					atomic_signal_fence(memory_order_release);
					atomic_store_explicit(&tail, tmp_tail, index_release_barrier);
				}
				return true;
			}
			...
			T* at(size_t index) {
				...
				if((atomic_load_explicit(&head, index_acquire_barrier) - tmp_tail) <= index)
					return nullptr;
				else
			...
			T& operator[](size_t index) {
				return data_buff[(atomic_load_explicit(&tail, memory_order_relaxed) + index) & buffer_mask];
			}
		...
		private:
			...
			constexpr static memory_order index_acquire_barrier = fake_tso ?
					  memory_order_relaxed
					: memory_order_acquire; // do not load from, or store to buffer before confirmed by the opposite side
			constexpr static memory_order index_release_barrier = fake_tso ?
					  memory_order_relaxed
					: memory_order_release; // do not update own side before all operations on data_buff committed

			alignas(cacheline_size) _Atomic(index_t) head; //!< head index
		alignas(cacheline_size) _Atomic(index_t) tail; //!< tail index
	...
	template<typename T, size_t buffer_size, bool fake_tso, size_t cacheline_size, typename index_t>
		...
		available = buffer_size - (tmp_head - atomic_load_explicit(&tail, index_acquire_barrier));
	...
	template<typename T, size_t buffer_size, bool fake_tso, size_t cacheline_size, typename index_t>
		...
		available = atomic_load_explicit(&head, index_acquire_barrier) - tmp_tail;
