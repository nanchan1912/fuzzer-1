  ...
  for (iter = 0; iter < afl->foreign_sync_cnt; iter++) {
    ...
    if (afl->foreign_syncs[iter].dir && afl->foreign_syncs[iter].dir[0]) {
      ...
      for (i = 0; i < (u32)nl_cnt; ++i) {
        ...
        afl->queued_imported += save_if_interesting(afl, mem, len, fault, 0, MAP_SIZE);
...
void read_testcases(afl_state_t *afl, u8 *directory) {
  ...
  parse_program_abstraction_file(afl->static_program_abstraction);
  ...
  if (nl_cnt) {
    ...
    do {
      ...
      // - Hardik
      if (afl->queue_top && afl->queue_top->graph_data) {
        afl->queue_top->graph_data->skeleton_graph = read_from_json(fn2);
      }
...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  if (afl->check_data_race) {
    ...
    fn = alloc_printf("%s/races", afl->out_dir);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (afl->check_data_race) {
    ...
    tmp = alloc_printf("%s/races", afl->out_dir);
    if (mkdir(tmp, afl->dir_perm)) { PFATAL("Unable to create '%s'", tmp); }
    ck_free(tmp);
