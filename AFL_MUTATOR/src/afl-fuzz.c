...
static void print_edge_frequency(EventTriple from, EventTriple to,
               uint32_t frequency, void* user_data) {

  (void)user_data;
  SAYF("  (%u, %llu, %u) -> (%u, %llu, %u) : %u times\n",
    from.thread_id, from.instruction_id, from.visit_id,
    to.thread_id, to.instruction_id, to.visit_id,
    frequency);
...
static void usage(u8 *argv0, int more_help) {
  ...
  #define PERSISTENT_MSG
    ...
    SAYF(
      ...
      "AFL_ENABLE_FEEDBACK: use simulator-provided next-event feedback when mutating\n"
      "AFL_CHECK_DATA_RACE: set happens-before data race checking (default: 1)\n"
      "AFL_POTENTIAL_LOCATIONS_FILE: path to location file for potential calculation\n"
      "AFL_INTERESTING_LOCATIONS_FILE: path to location file for interesting locations\n"
      "AFL_CUTOFF_PERCENTILE: percentile for cutoff in graph scoring (default: 1)\n"
      "AFL_SKELETON_GRAPH_HIGHEST_STEP: set the maximum skeleton graph step size (default: 3)\n"
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  set_skeleton_graph_rng_state(afl);
  ...
  while ((opt = getopt(
    ...
    switch (opt) {
      case 'v': 
        if(afl->static_program_abstraction){ FATAL("Multiple -v options not supported");}
        if(optarg == NULL){ FATAL("Invalid -v option (got NULL)."); }
        afl->static_program_abstraction = optarg;
        break;
  ...
  if (unlikely(fast_resume)) {
    ...
    if (afl->in_bitmap) {
      ...
      memset(afl->virgin_bits, 255, map_size + SKELETON_GRAPH_MAP_SIZE);

    }

    memset(afl->virgin_tmout, 255, map_size + SKELETON_GRAPH_MAP_SIZE);
    memset(afl->virgin_crash, 255, map_size + SKELETON_GRAPH_MAP_SIZE);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
          ...
          */
          ...
          if (unlikely(prev_queued_items < afl->queued_items ||
            ...
            if(afl->reinit_table){
              ...
              // ACTF("prev_queued_items: %u, afl->queued_items: %u", prev_queued_items, afl->queued_items);
            }
      ...
      if (afl->queue_cycle == 0 || ((afl->queue_cycle - 1) / 5) % 2 == 0) {
        afl->current_phase = MO_FOOTPRINT_DRIVEN_PHASE;
      } else {
        afl->current_phase = POTENTIAL_DRIVEN_PHASE;
      }
...
stop_fuzzing:
  ...
  if (afl->pizza_is_served) {
    ...
    //print RF edge exploration frequencies
    if (get_rf_coverage_count() > 0) {
      SAYF("\nRF Edge Exploration Frequencies:\n");
      for_each_rf_edge(print_edge_frequency, NULL);
    }
