...
inline u8 has_new_bits(afl_state_t *afl, u8 *virgin_map) {
  ...
  // u64 *current = (u64 *)(afl->fsrv.trace_bits + base);
  ...
  // u32 *current = (u64 *)(afl->fsrv.trace_bits + base);
...
u8 __attribute__((hot)) save_if_interesting(afl_state_t *afl, void *mem,
  ...
  save_to_queue:
    ...
    // TODO: later, uncomment the call in afl-fuzz-run
...
u8 save_if_interesting_skeleton(afl_state_t *afl, const struct SkeletonGraph *graph,
                                u8 fault) {
  ...
  switch (fault) {
    ...
    case FSRV_RUN_TMOUT:
      ...
      if (likely(!afl->non_instrumented_mode)) {
        ...
        simplify_trace(afl, afl->fsrv.trace_bits);
        virgin_map = afl->virgin_tmout;

        if (!has_new_bits(afl, virgin_map)) {
      ...
      snprintf(fn, PATH_MAX, "%s/hangs/id:%06llu,src:%06u.json", afl->out_dir,
           afl->saved_hangs, src_id);
      ++afl->saved_hangs;
      afl->last_hang_time = get_cur_time();
      break;
    ...
    case FSRV_RUN_CRASH:
      ...
      snprintf(fn, PATH_MAX, "%s/crashes/id:%06llu,src:%06u.json", afl->out_dir,
           afl->saved_crashes, src_id);
      ++afl->saved_crashes;
      afl->last_crash_time = get_cur_time();
      afl->last_crash_execs = afl->fsrv.total_execs;
      break;
